...
function investMoney($uid, $borrow_id, $money, $_is_auto = 0, $from = 1,$is_experience = 0, $member_interest_rate_id = 0, $bonus_id = 0, $money_bonus = 0, $borrow_pass = '')
{
    $pre = c("DB_PREFIX");
    $done = false;
    $datag = get_global_setting();
    ...
    $binfo = m("borrow_info")->lock(true)->field("borrow_uid,borrow_money,borrow_name,borrow_interest_rate,borrow_type,borrow_duration,borrow_duration,borrow_status,repayment_type,has_borrow,bespeak_able,start_time,bespeak_days")->find($borrow_id);
    if($binfo)
    {
...
function zinvestMoney($uid, $borrow_id, $money,$yubi,$_is_auto = 0, $from = 1,$is_experience = 0, $member_interest_rate_id = 0, $bonus_id = 0, $money_bonus = 0, $borrow_pass = '')
{
    ...
    $investMoney = d("borrow_investor");
    $investMoney->startTrans();
    $binfo = m("borrow_info")->lock(true)->field("borrow_uid,borrow_money,borrow_name,borrow_interest_rate,borrow_type,borrow_duration,borrow_duration,borrow_status,repayment_type,has_borrow")->find($borrow_id);
    if($binfo)
    {
        $vminfo = getminfo($uid, "m.user_leve,m.time_limit,mm.account_money,mm.back_money,mm.yubi");
...
function borrowApproved($borrow_id)
{
    ...
    $borrowInvestor = D("borrow_investor");
    $borrowInvestor->startTrans();
    $binfo = M("borrow_info")->field("borrow_type,reward_type,reward_num,borrow_name,borrow_fee,borrow_money,borrow_uid,borrow_duration,repayment_type")->find($borrow_id);
    $borrow_name=$binfo["borrow_name"];
...
function memberMoneyLog($uid, $type, $amoney, $info = "", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0)
{
    $xva = floatval($amoney);
    if (empty($xva)) {
        return true;
    ...
    $done = false;
    $MM = m("member_money")->field("money_freeze,money_collect,account_money,money_experience,yubi,yubi_freeze")->find($uid);
    if (!is_array($MM)) {
...
function memberCreditsLog($uid, $type, $acredits, $info = "", $id = "")
{
    if ($acredits == 0) {
        return true;
    }
...
function getMinfo($uid, $field = "m.pin_pass,mm.account_money,mm.money_experience,mm.back_money,IFNULL(m.nick_name,m.user_name) as nick_name")
{
    ...
    if(empty($auserLastLoginLog)){
        ...
        foreach($borrowinvestor as $key=>$value){
            ...
            if(!empty($value)){
                ...
                M('member_moneylog')->add(['uid'=>$value["uid"],'type'=>'9','affect_money'=>$receive_interest,'account_money'=>$money['account_money']+$receive_interest,'back_money'=>'0','collect_money'=>'0','freeze_money'=>'0','info'=>'体验金回款','add_time'=>time(),'add_ip'=>get_client_ip(),'target_uid'=>0,'target_uname'=>'','experience_money'=>$value['investor_capital']]);
                ...
                M('member_money')->save(["uid"=>$money['uid'],"money_freeze"=>$money['money_freeze']-$value['investor_capital'],"money_collect"=>$money['money_collect'],"account_money"=>$money['account_money']+$receive_interest,"back_money"=>$money['back_money'],"credit_limit"=>$money['credit_limit'],"credit_cuse"=>$money['credit_cuse'],"borrow_vouch_limit"=>$money['borrow_vouch_limit'],"borrow_vouch_cuse"=>$money['borrow_vouch_cuse'],"invest_vouch_limit"=>$money['invest_vouch_limit'],"invest_vouch_cuse"=>$money['invest_vouch_cuse'],"money_experience"=>$money['money_experience'],"update_time"=>time()]);
