...
function adminCreditsLog($type, $info = "无")

{
    ...
    $map['id'] = session("admin");

	$admin = M('ausers')->field('id,user_name,u_group_id,real_name,is_kf,area_id,user_word,phone')->where($map)->find();

    $Creditslog = d("admin_log");
   ...
   // $Creditslog->startTrans();
    ...
    $data['info_log'] = str_replace("[N]",$admin["user_name"],$info);
...
function user_names($uid){
    $minfo=M('members')->where('id='.$uid)->getField('user_name');
    return $minfo;
}
function getzengpin($zid){
    $minfo=M('zeng_pin')->where('id='.$zid)->getField('title');
    return $minfo;
}
