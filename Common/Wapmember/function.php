...
function getWithDrawLog($map,$size,$limit=10){
	...
	foreach($list as $key=>$v){
		$list[$key]['status_c'] = $status_arr[$v['withdraw_status']];
	}
...
function getChargeLog($map,$size,$limit=10){
	...
	foreach($list as $key=>$v){
		$list[$key]['status_c'] = $status_arr[$v['status']];
	}
...
function getBorrowList($map,$size,$limit=10){
	...
	if($size){
		//分页处理
		import("ORG.Util.Page");		
		$subQuery = $Model->field(true)->where($map)->order('times ASC')->group('id')->buildSql();
...
function getTenderList($map,$size,$limit=10){
	...
	if($size){
		...
		import("ORG.Util.Page");
		$count = D("TenderListView")->where($map)->order('invest_time desc,borrowid desc')->group('id')->count();
		$p = new Page($count, $size);
		$page = $p->show();
	//	var_dump($count,"<br>",$page,"1");die;
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	}else{
		$page="";
	...
	foreach($list as $key=>$v){
		...
		$field = "id,borrow_uid,borrow_money,borrow_status,borrow_type,has_borrow,has_vouch,borrow_interest_rate,borrow_duration,repayment_type,collect_time,borrow_min,borrow_img,borrow_max,password,borrow_use,deadline";
		$vo = M('borrow_info')->field($field)->find($list[$key]['borrow_id']);
		$list[$key]['borrow_status']=$vo["borrow_status"];
		// $list[$key]['investor_capital']=$v["investor_capital"];
		...
		$list[$key]['borrow_img']= $vo['borrow_img'];
		if($vo['borrow_status'] == 7){
			$list[$key]['hk_time'] = empty($vo['deadline'])?$vo['start_time']+$vo['borrow_time']*60*60*24:$vo['deadline'];
		}else{
			if(!empty($v['deadline'])){

				$list[$key]['hk_time'] = intval($v['deadline'])+(8*24*3600);
			}else{
				$list[$key]['hk_time'] = '';
...
function getBackingList($map,$size,$limit=10){
	...
	if($size){
		//分页处理
		import("ORG.Util.Page");
		$count = M('investor_detail')->where($map)->count('d.id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
