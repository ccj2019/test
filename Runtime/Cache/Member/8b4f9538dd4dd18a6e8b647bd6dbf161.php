...
<form action='__URL__/borrowpaying' id='searinfo' method='get'>
        ...
        <div class="tabv clearfix">
            <div <?php if($borrow_status == ''): ?>class="on"<?php endif; ?> ><span><a href="<?php echo U('borrowin/borrowpaying');?>">所有收益</a></span></div>
            <div <?php if($borrow_status == 'paydone'): ?>class="on"<?php endif; ?> ><span><a href="<?php echo U('borrowin/borrowpaying',array('borrow_status'=>'paydone'));?>">已发放</a></span></div>
            <div <?php if($borrow_status == 'paying'): ?>class="on"<?php endif; ?> ><span><a href="<?php echo U('borrowin/borrowpaying',array('borrow_status'=>'paying'));?>">未发放</a></span></div>
<div>

    <input name="name" size="30" type="text" id="name" value="<?php echo ($name); ?>">
...
<input name="borrow_status" type="hidden" value="<?php echo ($borrow_status); ?>">
        ...
        <div class="tabv2box">
            <div class="tabv2 show">
                <div class="bgbx">
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="Tzic">
                        <div class="clearfix Tzicson">
                            <h3><?php echo (cnsubstr($vo["borrow_name"],28)); ?></h3>
                            <div class="syLsis clearfix">
                                <p><span>融资金额：</span>¥<?php echo ($vo["borrow_money"]); ?></p>
                                <p><span>已筹金额：</span>¥<?php echo ($vo["borrow_money"]); ?></p>
                                <p><span>投资人数：</span><?php echo (getinvestnums($vo["id"])); ?></p>
                                <p><span>收益周期：</span><?php echo ($vo["borrow_duration"]); ?>个月</p>
                                <p><span>已发放期数：</span><?php echo ($vo["has_out"]); ?>期</p>
                                <p><span>已发放收益：</span>¥ <?php if($vo["borrow_status"] == '6'): ?>0.00<?php else: echo ($vo["repayment_interest"]); endif; ?></p>
                                <!-- <p><span>待发放收益：</span>¥<?php echo ($vo["borrow_money"]); ?></p> -->
                                <p><span>已发本金：</span>¥<?php if($vo["borrow_status"] == '6'): ?>0.00<?php else: echo ($vo["repayment_money"]); endif; ?></p>
                                
                            </div>
                            <div class="Fasy">
                                <span><?php if($vo["borrow_status"] == '7'): ?><a style="color: #fff;">已发放</a><?php else: ?><a href="<?php echo U('borrowin/fenhong',array('id'=>$vo['id']));?>" style="color: #fff;">发放收益</a><?php endif; ?></span>
                            </div>
                        </div>
                            <div class="XmBhbox clearfix">
                                ...
                                <?php $_result=get_invest_list($vo['id']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vb): $mod = ($i % 2 );++$i;?><div class="touTz1 ">
                                    <span><?php echo ($key+1); ?></span>
                                    <span><?php echo ($vb["sort_order"]); ?>期</span>
                                    <span><?php echo ($vo["borrow_duration"]); ?>个月</span>
                                    <span>¥<?php echo ($vb["income"]); ?></span>
                                    <!-- <span>11.50%</span> -->
                                    <?php if($vb["repayment_time"] == '0'): ?><span>&nbsp;</span>
                                    <?php else: ?>
                                    <span><?php echo (date("Y-m-d",$vb["repayment_time"])); ?></span><?php endif; ?>
                                    <?php if($vb["status"] == 1): ?><span>已通过</span>
                                    <!-- <span>查看明细</span> -->
                                    <?php else: ?>
                                    <span>等待审核</span>
...
<script>
    function chazhao(){
        var key=$("#name").val();
        window.location.href = "/borrowin/borrowpaying.html?borrow_status="+<?php echo ($borrow_status); ?>+"name="+key;
    }
</script>
<!--底部-->
<div class="clear"></div>
