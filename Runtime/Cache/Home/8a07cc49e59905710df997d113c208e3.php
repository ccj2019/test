...
<div class="list"> 
    <form action="__URL__/huikuan"  method="post" class="form form-horizontal" id="form-add" name="form-add" enctype="multipart/form-data">
     日期选择：<input type="text" onfocus="" name="time" class="input-text Wdate" id="time" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" 
    value='<?php echo (date('Y-m-d',$time)); ?>'> 
  <button class="btn btn-secondary radius" type="submit"> 查询</button>
</from>
    <br><br>
<?php if(is_array($tzlist)): $i = 0; $__LIST__ = $tzlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="/Member/borrowin/fenhong/id/<?php echo ($vo["id"]); ?>/qi/<?php echo ($vo["qi"]); ?>.html" target="_blank"><?php echo ($i); ?>. <?php echo ($vo["borrow_name"]); ?>   第<?php echo ($vo["qi"]); ?>期</a><br><?php endforeach; endif; else: echo "" ;endif; ?> 
</div>
        <!--底部-->
<div class="clear"></div>
