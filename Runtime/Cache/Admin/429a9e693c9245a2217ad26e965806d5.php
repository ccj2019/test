...
<div class="so_main">
  <div class="page_tit">初审待审核众筹</div>
<!--搜索/筛选会员-->
...
<!--搜索/筛选会员-->
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
		...
		<td><?php echo (date("Y-m-d H:i",$vo["lead_time"])); ?></td>
        <td>
              <?php if($vo["borrow_status"] == '0'): ?><a href="__URL__/edit?id=<?php echo ($vo['id']); ?>">审核</a> <?php else: ?>已审核<?php endif; ?> 
              <!--< elseif condition="$vo.borrow_status eq '5'"/> 1-->
        </td>
