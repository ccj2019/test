...
<!--搜索-->
  ...
  <div id="search_div" style="display:none">
	...
	<dl class="lineD"><dt>时间从：</dt><dd><input onclick="WdatePicker({maxDate:'#F{$dp.$D(\'end_time\')||\'2020-10-01\'}',startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true});" name="start_time" id="start_time"  class="input" type="text" value="<?php echo ($search["start_time"]); ?>"><span id="tip_start_time" class="tip">时间段</span></dd></dl>
	<dl class="lineD"><dt>到：</dt><dd><input onclick="WdatePicker({minDate:'#F{$dp.$D(\'start_time\')}',maxDate:'2020-10-01',startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true});" name="end_time" id="end_time"  class="input" type="text" value="<?php echo ($search["end_time"]); ?>"><span id="tip_end_time" class="tip">时间段</span></dd></dl>
...
<!--搜索-->
  ...
  <div class="Toolbar_inbox">
    ...
    <a class="btn_a" href="__URL__/exportall?<?php echo ($query); ?>"><span>整合起来并导出Excel</span></a>
  </div>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        ...
        <td><?php echo ($vo["xway"]); ?></td>
        <td><?php echo ($vo["money"]); ?></td>
        <td><?php echo (date("Y-m-d H:i:s",$vo["add_time"])); ?></td>
        <td><?php echo ($vo["status"]); ?></td>
        <td><?php echo (($vo["tran_id"])?($vo["tran_id"]):"无"); if($vo["way"] == '线下充值' or $vo["way"] == '网站充值' ): ?>入帐银行：<?php echo ($vo["off_bank"]); ?>##充值方式：<?php echo ($vo["off_way"]); endif; ?></td>
        <td><?php echo (($vo["deal_user"])?($vo["deal_user"]):"无"); ?></td>
        <td><?php echo ($vo["deal_info"]); ?></td>
        <td>

          <?php if($vo["status_num"] == 0): if( in_array($vo['way'],array('zfb','wx')) ): ?><a href="__URL__/dobulu?oid=<?php echo ($vo['nid']); ?>">补录验证</a>

              <?php else: ?>
              <a href="javascript:;" onclick="edit('?id=<?php echo ($vo["id"]); ?>')">操作</a><?php endif; ?>
