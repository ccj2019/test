...
<!--搜索/筛选会员-->
  ...
  <th class="line_l">用户名</th>
    ...
    <th class="line_l">众筹手续费</th>

      <th class="line_l">赠品管理</th>


    <th class="line_l">操作</th>
  </tr>
<?php if($isindex != 1): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
    ...
    <td><a onclick="loadUser(<?php echo ($vo["mid"]); ?>,'<?php echo ($vo["user_name"]); ?>')" href="javascript:void(0);"><?php echo ($vo["user_name"]); ?></a></td>
        <td>
    <a href="/invest/detail/id/<?php echo ($vo["id"]); ?>.html" title="<?php echo ($vo["borrow_name"]); ?>" target="_blank"><?php echo (cnsubstr($vo["borrow_name"],22)); ?></a>
    </td>
        <td><?php echo ($vo["borrow_money"]); ?></td>
        <td><?php echo ($vo["borrow_fee"]); ?></td>

          <?php if($vo["is_huodong"] == 1): ?><td><a href="__URL__/../zporder/index?id=<?php echo ($vo['id']); ?>">自提</a>/<a href="__URL__/../zporder/zindex?id=<?php echo ($vo['id']); ?>">折现</a></td>
              <?php else: ?>
              <td> ----- </td><?php endif; ?>



          <td><a href="__URL__/view?id=<?php echo ($vo['id']); ?>">投标记录</a>
         | <!-- <a href="__URL__/vote?id=<?php echo ($vo['id']); ?>">投票管理</a> | --> <a href="__URL__/edit?id=<?php echo ($vo['id']); ?>">项目跟踪</a>&nbsp;<!-- |&nbsp;<a href="__URL__/dorepayment?id=<?php echo ($vo['id']); ?>">回报计划</a> --></td>
      </tr><?php endforeach; endif; else: echo "" ;endif; ?>
  <?php else: ?>
   <?php if(is_array($list1)): $i = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
        <td><input type="checkbox" name="checkbox" id="checkbox2" onclick="checkon(this)" value="<?php echo ($vo["id"]); ?>"></td>
        <td><?php echo ($vo["id"]); ?></td>
    <td><a onclick="loadUser(<?php echo ($vo["mid"]); ?>,'<?php echo ($vo["user_name"]); ?>')" href="javascript:void(0);"><?php echo ($vo["user_name"]); ?></a></td>
        <td>
    <a href="/invest/detail/id/<?php echo ($vo["id"]); ?>.html" title="<?php echo ($vo["borrow_name"]); ?>" target="_blank"><?php echo (cnsubstr($vo["borrow_name"],22)); ?></a>
        ...
        <td><?php echo ($vo["borrow_fee"]); ?></td>
         <td><?php echo (date("Y-m-d H:i",$vo["deadline"])); ?></td>
          <td><a href="__URL__/view?id=<?php echo ($vo['id']); ?>">投标记录</a>&nbsp;|&nbsp;
          <!-- <a href="__URL__/vote?id=<?php echo ($vo['id']); ?>">投票管理</a> |  -->
          <a href="__URL__/edit?id=<?php echo ($vo['id']); ?>">项目跟踪</a> | 
        <!-- <a href="__URL__/dorepayment?id=<?php echo ($vo['id']); ?>">回报计划</a> --></td>
