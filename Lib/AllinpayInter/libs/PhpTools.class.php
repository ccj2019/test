...
class PhpTools{
	...
	const privateKeyFile = './data/20058400001872504.pem';//商户私钥证书
	const password = '111111';//商户私钥密码以及用户密码
	public $arrayXml ;
    // const apiUrl = 'https://113.108.182.3/aipg/quickpay';//通联系统对接请求地址（外网,商户测试时使用）
	const apiUrl = 'https://tlt.allinpay.com/aipg/quickpay';//（生产环境地址，上线时打开该注释）
	...
	public function verifyXml($xmlResponse){
		...
		$pubKeyId = openssl_get_publickey(file_get_contents(dirname(__FILE__).'/../'.PhpTools::certFile));
...
//		    echo $results;
	...
	public function signXml($params){
		...
		$privateKey = file_get_contents(dirname(__FILE__).'/../'.PhpTools::privateKeyFile);
	...
	public function send($params){
		...
		file_put_contents('1.xml', $xmlSignPost );
...
//		print_r(str_replace('<','&lt;',$xmlResponse));
		...
		file_put_contents('2.xml', $result );
		return $result;
	}
}
