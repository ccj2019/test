   ...
   class BondBehavior
   {
       ...
       */
        ...
        */
        ...
        {  
            $count = M('borrow_investor')->where("investor_uid = ".$this->uid."  and status = 4 and Bond_status=0")->count();
            ...
            $transfer = M('borrow_investor i')
                ...
                ->where("i.investor_uid = ".$this->uid."  and i.status = 4 and i.Bond_status=0")
                ->limit($Page->firstRow. ',' . $Page->listRows)
                ->order('i.id')
                ->select();
         
            foreach($transfer as $k=>$v){
               $arr = $this->countBond($v['id']);
        ...
        */
        public function countBond($invest_id)
        {
            $Bond = array();
           ...
           //可转让期数、统计待收本金和利息
           $Bond = M("investor_detail")->field("count(id) as re_num, sum(capital) as capital, sum(interest) as interest ")->where($condition)->find();
           $Bond['total'] = M("investor_detail")->where("invest_id=".$invest_id)->count('id'); //总共多少期
           return $Bond;
        ...
        */
        public function sell($invest_id, $price, $paypass)
        {
            ...
            if($check==='TRUE'){ // 检测通过
                $count_invest = $this->countBond($invest_id);
                ...
                if($record){
                    $Bond = $this->Model->where("id=".$record)->save($info);      
                }else{
                    $Bond = $this->Model->add($info);
                }
				//echo M('invest_bond')->getlastsql();
                $investor = M("borrow_investor")->where("id=".$invest_id)->save(array('Bond_status'=>1));
                if($Bond && $investor){
                   $this->Model->commit();
                   return 'TRUE';
				   // return '债权转让成功';
        ...
        */
        private function checkSell($invest_id, $price, $paypass)
        {
            ...
            $datag = get_global_setting();
            $datag['Bond_fee'];
            $fee = round($price * $datag['Bond_fee'] / 100, 2);
        ...
        */
        ...
        {    
            $count = M('invest_bond')->where("sell_uid = ".$this->uid."  and status in (2,99)")->count();
            $Page = new Page($count, $this->pagesize);
            $Bonds['data'] = M('invest_bond d')
        ...
        */
        public function cancel($invest_id, $paypass)
        {
            ...
            if($this->cancelBond($invest_id, 1)){
               return true; 
            }else{
                return false;
            }
        ...
        */
        public function cancelBond($invest_id, $type)
        {
            ...
            $condition1 = " id={$invest_id} and Bond_status=1";
            $condition2 =  " invest_id={$invest_id} and status=2";
            $this->Model->startTrans();
            $borrow_investor = M("borrow_investor")->where($condition1)->save(array("Bond_status"=>'0')); 
            $invest_bond = $this->Model->where($condition2)->save($update);
            if($borrow_investor && $invest_bond){
                $this->Model->commit();
                return true;
            }else{
                $this->Model->rollback();
        ...
        {  
            $count = M('invest_bond')->where("sell_uid = ".$this->uid."  and status = 3")->count();
            $Page = new Page($count, $this->pagesize);
            $Bonds['data'] = M('invest_bond d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        * 
        */
        public function successBond()
        {  
            $count = M('invest_bond')->where("sell_uid = ".$this->uid."  and status in (1,4)")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_bond d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        ...
        {
            $count = M('invest_bond')->where("buy_uid = ".$this->uid."  and status in (1,4)")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_bond d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        ...
        {
            $count = M('invest_bond')->where("buy_uid = ".$this->uid."  and status = 1")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_bond d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        public function listAll($parm,$size=0, $pstatus=true)
        {
            ...
            $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as Bond_id, i.id as invest_id,
            ...
            if($pstatus){ 
                //$count = M("invest_bond")->where("status in (2, 4)")->count();
				$count = M("invest_bond d")->where($map)->count();
                $Page = new Page($count, $this->pagesize); 
                $list['data'] = M("invest_bond d")
                        ->join("{$this->pre}borrow_investor i ON d.invest_id=i.id")
                        ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        public function buy($paypass, $invest_id)
        {
            ...
            if($check_result === 'TRUE'){
                $Bond_info = $this->Model->field("transfer_price, sell_uid, money")->where("invest_id={$invest_id}")->find();
                $buy_user = M("member_money")->field("account_money, back_money, money_collect")->where("uid={$this->uid}")->find();
                $sell_user = M("member_money")->field("account_money, money_collect")->where("uid={$Bond_info['sell_uid']}")->find();
                if($Bond_info['transfer_price'] > ($buy_user['account_money']+$buy_user['back_money'])){
                    return 'insufficient_account_balance';
                }   
                
                $buy_user['back_money'] = $buy_user['back_money'] - $Bond_info['transfer_price'];
                if($buy_user['back_money'] < 0){
                    $buy_user['account_money'] = $buy_user['account_money'] + $buy_user['back_money'];
                    $buy_user['back_money'] = 0;
                }
                $buy_user['money_collect'] += $Bond_info['money']; 
                // 转让手续费扣除
                $datag = get_global_setting();
                $Bond_fee = $datag['Bond_fee'];// 百分比
                $fee = round(($Bond_info['transfer_price'] * $Bond_fee)/100, 2);
                //转让方增加资金
                $sell_user['account_money'] += $Bond_info['transfer_price'];//$Bond_info['transfer_price']-$fee;该处不能扣减手续费，因为在转让成功时扣除了手续费，这里扣就重复了
                $sell_user['money_collect'] = $sell_user['money_collect'] - $Bond_info['money'];// 减去待收
                ...
                $this->Model->startTrans();
                $investor_status = M("borrow_investor")->where("id={$invest_id}")->save(array("Bond_status" => 2, 'Bond_uid'=>$this->uid));
                $detail_status = M("investor_detail")->where("invest_id={$invest_id} and status in (6,7)")->save(array('investor_uid'=>$this->uid));
                $Bond_status = $this->Model->where("invest_id={$invest_id}")->save($Bond);
                $sell_user_status = M("member_money")->where("uid={$Bond_info['sell_uid']}")->save($sell_user);
                $buy_user_status = M("member_money")->where("uid={$this->uid}")->save($buy_user);
                if($investor_status && $detail_status && $Bond_status && $sell_user_status && $buy_user_status){
                    $this->Model->commit();
                    $this->moneyLog($this->uid, 46, -$Bond_info['transfer_price'], $Bond_info['money'], "购买{$serialid}号债权", $Bond_info['sell_uid']);
                    $this->moneyLog($Bond_info['sell_uid'], 47, $Bond_info['transfer_price'], $Bond_info['money'], "转让{$serialid}号债权", $this->uid);
                    memberMoneyLog($Bond_info['sell_uid'],48,-$fee,"转让{$serialid}号债权手续费（转让金额的{$Bond_fee}%）");
                    //return 'TRUE';  
					return '购买成功';
                }else{
                    $this->Model->rollback();
        ...
        * @param float $$Bond_money // 债权金额
        * @param string $info //日志说明
        * @param int  $target_uid // 交易对方uid
        */
        private function moneyLog($uid, $type, $money, $Bond_money, $info, $target_uid)
        {
            ...
            if($money > 0){  // 增加资金
                ...
                $money_log['collect_money'] = $money_log['collect_money'] - $Bond_money;//待收资金减少债权金额
                $money_log['affect_money'] = -$Bond_money;
                ...
                M("member_moneylog")->add($money_log);
                $money_log['collect_money'] += $Bond_money;//待收资金增加债权金额
                $money_log['affect_money'] = $Bond_money;
        ...
        */
        public function adminList($status='1')
        {
            ...
            $list['data'] = M("invest_bond d")
                ...
                ->join("{$this->pre}borrow_info b ON i.borrow_id= b.id")
                ->join("{$this->pre}members m ON d.sell_uid=m.id")
                ->field("d.id as Bond_id, d.remark, d.cancel_time, d.invest_id, d.status, m.user_name, b.borrow_name, b.id, d.addtime, b.borrow_interest_rate, b.total, b.has_pay, d.period, d.transfer_price")
                ->where("1 and {$status}")
                ->limit($Page->firstRow .','. $Page->listRows) 
