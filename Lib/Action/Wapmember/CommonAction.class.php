...
public function selectreg_code(){
    ...
    public function weixinregaction(){
        ...
        $data['user_name'] = trim(text($_POST['user_name']));
        $data['user_phone'] = trim(text($_POST['txtPhone']));
        $data['openid'] =  $_POST['openid'];
        $code = trim(text($_POST['code']));

        $user_namecount=M('members')->where("user_name = '{$data['user_name']}'")->count();
        if($user_namecount>=1){
                if($user_namecount>=10){
                    $data['user_name']=$data['user_name'].'00'.($user_namecount+1);
                }else  if($user_namecount>=100){
                    $data['user_name']=$data['user_name'].'0'.($user_namecount+1);
                }else  if($user_namecount>=1000){
                    $data['user_name']=$data['user_name'].($user_namecount+1);
                }else{
                    $data['user_name']=$data['user_name'].'000'.($user_namecount+1);
                }
        }

        $me=M('members')->where("(user_phone = '{$data['user_phone']}' OR user_name='{$data['user_name']}') and openid!=''")->find();
          //writeLog(M("members")->getlastsql());       
 if(!empty($me)){
            $json['status'] = "n";
        ...
        $mev=M('members')->where("user_phone = {$data['user_phone']}")->find();

        if(isset($mev)&&empty($mev['openid'])){
            $newid=$mev["id"];
           $me=M('members')->where("user_phone = '{$data['user_phone']}' ")->save(["openid"=>$data['openid']]);
        //  writeLog(M("members")->getlastsql());
            if($me){
//                session('u_id',$mev["id"]);
//                session('u_user_name',$mev['user_name']);
                $this->_memberlogin($mev['id']);
                //绑定微信的时候不应该添加体验金了
                if($this->glo['award_reg']>0){
                    $r = M('member_experience')->where('uid='.$newid)->find();
                    if(empty($r)){
...
//             exit(json_encode($json));
    ...
    public function curl(){ 
        ...
        $_SESSION["vazvcodedata"]=$aa;
        exit(json_encode($UserIP));
        print_r(1);
        exit;
     }
    public function sendcode() {
      ...
      $zz=session("vazvcodedata");
        $vazvcodedata =  $aa =  json_decode($zz, true);
