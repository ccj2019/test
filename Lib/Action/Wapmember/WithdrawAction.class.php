...
if(isset($_GET["bankid"])&&!empty($_GET["bankid"])){
	$vobank = M("member_banks")->where("(uid={$this->uid}) and (id=".$_GET["bankid"].")")->find();	
}else{
	$vobank = M("member_banks")->where("(uid={$this->uid}) and (is_default=1)")->find();	
		...
		if(empty($vo['bank_num'])){
				...
				echo "<script>alert('您还未绑定银行账户，请先绑定');  	window.location='/wapmember/bank/bank'   </script>";
//			$this->error('您还未绑定银行账户，请先绑定',__APP__.'/wapmember/bank/index.html');
			// echo '<script type="text/javascript">layer.msg("您还未绑定银行账户，请先绑定");setTimeout("window.location.href=\''.__APP__.'/member/bank/index.html\'",2000);</script>';
		}
		else{
...
$va = getFloatvalue($MM['account_money'],2);
//var_dump($va);die;
		$this->assign("va",$va);
	...
	public function actwithdraw(){
		...
		*/
		...
		// $fee += $needFeeMoney * getfloatvalue($tqfee[4],2) / 1000;
		$fee = 0.00;	
		$member_banks = M('lzh_member_banks') ->where("id = ".$bank_id) ->find();	
//		id
//uid
//bank_id
//bank_num
