    ...
    */
    ...
    public function index()
    {
		...
		if(!empty($_REQUEST['start_time']) && !empty($_REQUEST['end_time'])){
			$startime=strtotime(urldecode($_REQUEST['start_time']));
			$endtime=strtotime(urldecode($_REQUEST['end_time']));	
		}elseif(!empty($_REQUEST['start_time'])){
			$startime=strtotime(urldecode($_REQUEST['start_time']));
			$endtime=strtotime(date('Y-m-d 23:59:59',strtotime('+365 day')));	
		}elseif(!empty($_REQUEST['end_time'])){
			$startime=time();
			$endtime=strtotime(urldecode($_REQUEST['end_time']));	
		}else{
			$startime=strtotime(date('Y-m-d 23:59:59',strtotime('-1 day')));
		...
		foreach($list as $key=>$val){
			...
			switch ($val['repayment_type']) {
				case 1 :
					$fields = "borrow_id,sort_order,sum(capital) as capital,sum(interest) as interest,status,sum(receive_interest+receive_capital+if(receive_capital>0,interest_fee,0)) as paid,deadline";
					$vo = m("investor_detail")->field($fields)->where("borrow_id={$val[id]} AND `sort_order`=1")->group("sort_order")->find();
					$status_arr = array("还未还", "已还完", "已提前还款", "愈期还款", "网站代还本金");
					$vo['status'] = $status_arr[$vo['status']];
					if($startime>$vo['deadline'] or $endtime<$vo['deadline'])continue;
					$vo['needpay'] = getfloatvalue(sprintf("%.2f", $vo['interest'] + $vo['capital'] - $vo['paid']), 2);
					$allmoenys+=$vo['needpay'];
					$list_all[$val['id']]=$val;
					$list_all[$val['id']]["rpay"][] = $vo;
					break;
				case 5:
					$fields = "borrow_id,sort_order,sum(capital) as capital,sum(interest) as interest,status,sum(receive_interest+receive_capital+if(receive_capital>0,interest_fee,0)) as paid,deadline";
					$vo = m("investor_detail")->field($fields)->where("borrow_id={$val[id]} AND `sort_order`=1")->group("sort_order")->find();
				...
				default :
					$i = 1;
					for (; $i <= $val['borrow_duration']; ++$i) {
						//echo $val["has_pay"];
						if($val["has_pay"]+1!=$i) continue;
					    $fields = "borrow_id,total,sort_order,sum(capital) as capital,sum(interest) as interest,status,sum(receive_interest+receive_capital+if(receive_capital>0,interest_fee,0)) as paid,deadline";
						$vo = m("investor_detail")->field($fields)->where("borrow_id={$val[id]} AND `sort_order`={$i}")->group("sort_order")->find();
						$status_arr = array("还未还", "已还完", "已提前还款", "愈期还款", "网站代还本金");
						$bigtime=$vo['deadline'];
						...
						//echo date("Y-m-d H:i:s",$vo['deadline']);
						//1  2  3
						// 1<=2 and 3>=2
						if($startime>$vo['deadline'] or $endtime<$vo['deadline'])continue;
						$list_all[$val['id']]=$val;
		...
		//print_r($list_all);
		$list_all = $this->_listFilter($list_all);
        $this->assign("bj", array("gt"=>'大于',"eq"=>'等于',"lt"=>'小于'));
        $this->assign("list", $list_all);
		 $this->assign("allmoenys", $allmoenys);
	 ...
	 public function doweek()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.deadline,m.user_name,m.id mid,b.repayment_money,b.total,b.has_pay';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
