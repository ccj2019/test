<?php
// 全局设置
class ProductcategoryAction extends ACommonAction
{
	var $typeleve=1;
    ...
    */
    ...
    {    	
		...
		$this->_list(D('Productcategory'),$field,$where,'sort_order');
    ...
    public function listType()
    {
		...
		$sonlist = D('Productcategory')->field(true)->where("parent_id={$typeid}")->select();
	...
	public function _doDelFilter($id){
		$n = D('Productcategory')->where("parent_id in ({$id})")->count();
		if($n==0) $n = D('Productcategory')->where("id in ({$id}) AND is_sys=1")->count();
		if($n>0){
	...
	protected function _typeLeve($typeid){
		...
		$condition['id'] = $typeid;
		$v = D('Productcategory')->field('parent_id')->where($condition)->find();
		if($v['parent_id']>0){
			$this->typeleve++;
	...
	protected function _typeSon($typeid){
		...
		$v = D('Productcategory')->field('id')->where($condition)->find();
		if($v['id']>0){
			return true;
		}else{
			return false;
