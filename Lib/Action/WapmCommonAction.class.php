<?php
// 全局设置
class WapmCommonAction extends Action
{
	var $glo=NULL;
	var $uid=0;
	//上传参数
    ...
    if(session("user_name")&&session("id")){
		...
		if($this->notneedlogin === true){
			...
			if(session("u_id")){
				...
				if(!in_array(strtolower(ACTION_NAME),array('regsafe','changephoneact','ckphone','ckphonecode','ckcode','sendcode',"actlogout",'emailactive','emailvalided','emailvsend1','regsuccess','emailverify','verify','borrow','test'))) redirect(__APP__."/wapmember/");
			}else{
				...
				if(substr(md5($loginconfig['cookie']['key'].$de_val),14,10) == cookie('Ukey2')){
					...
					if(is_array($vo)){
						...
						$unread=M("inner_msg")->where("uid={$this->uid} AND status=0")->count('id');
						$this->assign('unread',$unread);
						if(!in_array(strtolower(ACTION_NAME),array("actlogout",'emailactive','regsuccess','emailverify','verify'))) redirect(__APP__."/wapmember/");//20181103zzwedit
					}else{
						cookie("Ukey",NULL);
						cookie("Ukey2",NULL);
					}
