...
class ToolAction extends HCommonAction {
    ...
    public function index(){
		if(!$_POST){
		  ...
		  $this->assign('m',$_GET["month"]==""?6:$_GET["money"]);
		  $this->assign('interest_rate',$_GET["interest_rate"]==""?15:$_GET["interest_rate"]);
		  if($_GET["selDays"]=="")$_GET["selDays"]=$_GET["month"];
		  $this->assign('selDays',$_GET["selDays"]==""?6:$_GET["selDays"]);
		  $this->assign('money',$_GET["money"]==""?1000:$_GET["money"]);
		}
		if($_POST){
			...
			$repay_detail['jiangli'] = getFloatValue($money*$_POST["jiangli"]/100,2);
			$repay_detail['free'] = getFloatValue(($repay_detail["repayment_money"]-$money)*$_POST["free"]/100,2);
			$this->assign('repay_detail',$repay_detail);
			$this->assign('m',$month);
			$this->assign('interest_rate',$rate);
			$this->assign('selDays',$selDays);
    ...
    public function tool2(){
		...
		if($_POST){
			...
			//print_r($monthData);
			//print_r($repay_detail);
			//exit;
			$repay_detail['jiangli'] = getFloatValue($money*$_POST["jiangli"]/100,2);
			$repay_detail['free'] = getFloatValue(($repay_detail["repayment_account"]-$money)*$_POST["free"]/100,2);
			$this->assign('repay_detail',$repay_detail);
			$this->assign('m',$month);
			$this->assign('interest_rate',$rate);
			$this->assign('selDays',$selDays);
			...
			$data['html_1'] = $this->fetch('tool2_res_1');
			$data['html_2'] = $this->fetch('tool2_res_2');
			exit(json_encode($data));
		}
		$this->display();
		...
		if(!$_POST){
		//echo $_GET["money"];
		  $this->assign('m',$_GET["month"]);
		  $this->assign('interest_rate',$_GET["interest_rate"]);
		  if($_GET["selDays"]=="")$_GET["selDays"]=$_GET["month"];
		  $this->assign('selDays',$_GET["selDays"]);
    ...
    public function tool3(){
	 ...
	 public function home_ajax(){
		...
		$rate = floatval($_REQUEST['interest_rate']);
