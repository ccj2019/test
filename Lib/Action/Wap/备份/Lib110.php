...
function memberMoneyLog($uid, $type, $amoney, $info = "", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0,$benjin=0)
{
    ...
    $done = false;
    $MM = m("member_money")->field("money_freeze,money_collect,account_money,money_experience,yubi,yubi_freeze,yongjin")->find($uid);
    if (!is_array($MM)) {
...
function getMinfo($uid, $field = "m.is_ssq,m.pin_pass,mm.account_money,mm.yubi,mm.money_experience,mm.back_money,IFNULL(m.nick_name,m.user_name) as nick_name")
{
