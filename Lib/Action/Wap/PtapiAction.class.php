 ...
 * @funciton login 登录goodsdetail

 */
// require_once './App/Lib/Action/Wap/LogisticsAction.class.php';

class PtapiAction extends Action {
    ...
    function _initialize(){
        ...
        if($_REQUEST["uid"]){
            ...
            $this->uid = $_REQUEST['uid'] = $_POST['uid'] = $_GET['uid'] =  $this->jwt->verifyToken($_REQUEST["uid"]); 
        
        }

        // if($_REQUEST['upuid']){
        //     $_REQUEST['upuid'] = $this->jwt->verifyToken($_REQUEST["upuid"]); 
        // }
    }
     ...
     * 创建拼团
     */
    public function tgtuan(){
        $uid=$_REQUEST["uid"];
        $gid=$_REQUEST["gid"];
        $ginfo=M("pt_goods")->where("id=".$gid)->find();
...
//        }
        ...
        $data["gid"]=$gid;
        $data["uid"]=$uid;
        $data["title"]=$ginfo["title"];
        $data["type"]=$_REQUEST["type"];//1购买团、2推广团
        $data["num"]=0;
        $data["money"]=$ginfo["yongjin"];//佣金
        $data["zengjia"]=$ginfo["zengjia"];//单人佣金
        $data["price"]=$ginfo["price"];
        $data["image"]=unserialize($ginfo['images'])[0]['img'];
        $data["time"]=time();
        $data["end_time"]=strtotime("+".$ginfo['shixiao']." hour",time());
        $data["cnum"]=$ginfo["renshu"];
        $data["tuan_type"]=$ginfo["tuan_type"];
        $res=M("pt_tuan")->add($data);
        if($res){
            $jsons["msg"]="创建成功！";
            $jsons["status"]=1;
            $jsons["id"]=$res;
     ...
     * 用户推广开团列表
     */
    public function tgtuanlist(){
        //拼团超时
        $uid=$_REQUEST["uid"];
        $tuans = M('pt_tuan')->where(array('uid'=>$uid,'status'=>1,'end_time'=>array('lt',time())))->select();
        foreach($tuans as $kt=>$vt){
            // $this->autotuikuan($vt['id']);
        }

		
        // $tid=$_REQUEST["tid"];
        // $goods_id = M('pt_tuan')->where(array('id'=>$tid))->getField('gid');
        // $renshu = M('pt_goods')->where(array('id'=>$goods_id))->getField('renshu');
        // $gmap["end_time"]=array("lt",time());
        // $gmap["status"]=1;
        // $gmap["id"]=$tid;
        // $gmap["num"]=array('lt',$renshu);
        // M('pt_tuan')->where($gmap)->setField('status','3');
        ...
        $list=M('pt_tuan')->where($map)->limit($limit)->order($order)->select();

        foreach($list as $k=>$v){
            $list[$k]['shifu'] = M('pt_order')->where(array('uid'=>$uid,'tid'=>$v['id']))->getField('money');
            $hongbao = M('pt_hongbao')->where(array('uid'=>$uid,'tid'=>$v['id']))->getField('hongbao');
            $list[$k]['hongbao'] = $hongbao?$hongbao:0;
        }
        if(count($list)==$size){
            $limits=($page+1)*$size.','.$size;
            $countlist=M('pt_tuan')->where($map)->limit($limits)->order($order)->count();
            if($countlist>0){
                $jsons['data']["ispage"]=1;
            }else{
                $jsons['data']["ispage"]=0;
    ...
    public function gettuiguang(){
        ...
        $tid=$_REQUEST["tid"];
        $goods_id = M('pt_tuan')->where(array('id'=>$tid))->getField('gid');
        $goods = M('pt_goods')->where(array('id'=>$goods_id))->find();
        $renshu = $goods['renshu'];
        $gmap["end_time"]=array("lt",time());
        $gmap["status"]=1;
        $gmap["id"]=$tid;
        ...
        $goods_uid = M('pt_tuan')->where(array('id'=>$tid))->getField('uid');
        $user = M('member_info')->where(array('uid'=>$goods_uid))->select();

        $order = M('pt_order')->where(array('tid'=>$tid))->select();
        $uid = array_column($order,'uid');
        // var_dump($uid);exit;
        $members = M('member_info')->where(array('uid'=>array('in',$uid)))->select();

        //团信息
        $tinfo=M("pt_tuan")->where("id=".$tid)->find();

        $jsons['tuan']= $user;
        $jsons['members']= $members;
        $jsons['data']= $tinfo;
        $jsons['goods']= $goods;
        $jsons['status'] = '1';
        outJson($jsons);
     ...
     * 商品信息
     */
    public function goodsdetailorder(){
        $gid = $_REQUEST['gid'];
        $uid = $_REQUEST['uid'];
        $tid = $_REQUEST['tid'];
        // $type = $_REQUEST['type'];

        $ginfo = M('pt_goods')->where(array('id'=>$gid))->find();
        $ginfo['images'] = unserialize($ginfo['images']);
        $addr = M('member_address')->where(array('uid'=>$uid,'default'=>1))->find();
        if(empty($addr)){
            $addr = M('member_address')->where(array('uid'=>$uid))->find();
        }

        $hongbao = M('pt_hongbao')->where(array('tid'=>$tid,'uid'=>$uid,'type'=>0))->getField('hongbao');

        $yongjin = M('member_money')->where(array('uid'=>$uid))->getField('yongjin');

        outJson(array('goods'=>$ginfo,'addr'=>$addr,'hongbao'=>$hongbao?$hongbao:0,'yongjin'=>$yongjin?$yongjin:0));
    }
    ...
    public function ptorder(){
        ...
        if($zftype==1){
            ...
            if($minfo["pin_pass"]!=md5($_POST["pin_pass"])){
                $jsons['msg']= "支付密码错误！";
                $jsons['status'] = '0';
                outJson($jsons);
            }
        ...
        if($savedata['type']==0){
            $this->autotuikuan($savedata['tid']);
            $data = $this->pinttuanorder($savedata);
        }else{
            $data = $this->xiadan($savedata);
        }
    ...
    //推广团下单
    public function pinttuanorder($savedata){
        // $savedata = textPost($_REQUEST);
        ...
        $tinfo=M("pt_tuan")->where("id=".$savedata['tid'])->find();
        if($tinfo["end_time"]<time()){
            $jsons['msg'] ="拼团信息已过期！";
            $jsons['status'] = '0';
            return $jsons;
        }
        if($tinfo["status"]!=1&&$tinfo["tuan_type"]!=1){
            $jsons['msg'] ="拼团信息已过期！";
            $jsons['status'] = '0';
            return $jsons;
        }
        ...
        $gid=$savedata["gid"];
        $ginfo=$pt_goods->where("id=".$gid)->find();
        if($ginfo['kucun']<$savedata['num']){
            $jsons['msg'] ="库存不足";
            $jsons['status'] = '0';
            return $jsons;
        }

        $savedata['title'] =$tinfo["title"];
        $savedata['type'] =$tinfo["type"];
        $savedata['uid'] = $uid;
        $savedata["num"]=abs($savedata["num"]);
        $savedata["money"]=$ginfo["price"]*$savedata["num"];
        $savedata['pay_way'] = 1;//微信
        $savedata['tid'] = $savedata['tid'];
        $savedata['add_time'] = time();
        $savedata['add_ip'] = get_client_ip();
        $savedata['ordernums']=sprintf('%s-%s-%s', 'NDD', $uid,time());

        $hongbao = M('pt_hongbao')->where(array('uid'=>$uid,'tid'=>$tinfo['id']))->find();
        if($hongbao){
            $savedata['hbid']=$hongbao["id"];
        }
        if($savedata['zftype']==1){
            if($hongbao){
                $savedata['pay_yongjin']=$savedata['money']-$hongbao["hongbao"];
            }else{
                $savedata['pay_yongjin']=$savedata['money'];
            }
        }

//        $savedata['zftype']=1;
        // $savedata['liuyan'] =$_REQUEST["liuyan"];
        //hbid
        $newid = M('pt_order')->add($savedata);
        ...
        $pt_goods->startTrans();
        $ptgd=M("pt_goods")->lock(true)->where("id=".$gid)->find();
        $dat["kucun"]=$ptgd["kucun"]-$savedata["num"];
        $reskc=$pt_goods->where("id=".$gid)->save($dat);
        if($newid&&$reskc){
            $pt_goods->commit();
    ...
    //购买成团
    public function xiadan($savedata)
    {
        /**
         * 成团
         */
...
//        $data["cnum"]=$ginfo["renshu"];
//        $data["tuan_type"]=$ginfo["tuan_type"];
//        $hongbao = M('pt_hongbao')->where(array('uid'=>$order['uid'],'tid'=>$order['tid']))->find();
//        $res=M("pt_tuan")->add($data);
        ...
        $savedata['image'] =$ginfo["image"];
        $savedata['title'] =$ginfo["title"];
        //$savedata['type'] =$tinfo["type"];

        $savedata['uid'] = $uid;
        $savedata["num"]= abs($num);
        $savedata["money"]=$ginfo["price"]*$savedata["num"];
        $savedata['pay_way'] = $savedata['paytype'];//微信
        //$savedata['tid'] = $res;
        $savedata['add_time'] = time();
        $savedata['add_ip'] = get_client_ip();
        $savedata['ordernums']=sprintf('%s-%s-%s', 'NDD', $uid,time());
...
//        var_dump(M("pt_tuan")->getLastSql());
//        var_dump(M('pt_hongbao')->getLastSql());
//        var_dump($savedata);exit;
    ...
    //支付
    public function ptweixin(){

        include './WechatAction.class.php';

        $id = intval($_REQUEST['oid']);
        $gid =intval($_REQUEST['gid']);
        $type = intval($_REQUEST['paytype']);
        // $hbid = intval($_REQUEST['hbid']);
        $order = M('pt_order')->where(array('id'=>$id))->find();
        $goods = M('pt_goods')->where(array('id'=>$gid))->find();
        $member = M('members')->where(array('id'=>$order['uid']))->find();

        $hongbao = M('pt_hongbao')->where(array('uid'=>$order['uid'],'tid'=>$order['tid']))->find();
        $member_money = M('member_money')->where(array('uid'=>$order['uid']))->find();
        // $pay_yongjin = 0;
        if($type==3){
            $pay_money_one = $order['money']+$order['yunfei']-$hongbao['hongbao'];

            //判断红包金额 防止红包金额大于商品金额出现无法支付
            if($pay_money_one<=0){
                $pay_money_one = 0.01;
            }

            $pay_money_two = $member_money['yongjin']-$pay_money_one;
      
            //判断佣金是足够
            if($pay_money_two<=0){
                $pay_money = abs($pay_money_two) * 100;
                $pay_yongjin = $member_money['yongjin'];
            }else{
                $pay_money = 1;
                $pay_yongjin = $pay_money_one - 0.01;
            }
            M()->query("UPDATE lzh_pt_order set `pay_yongjin`=$pay_yongjin WHERE `id`=$id");
        }else if($type==1){
            $pay_money = ($order['money']+$order['yunfei']-$hongbao['hongbao'])*100;
        ...
        if(!empty($hongbao)){
            $hbid = $hongbao['id'];
            M()->query("UPDATE lzh_pt_order set `hbid`=$hbid WHERE `id`=$id");
        }
        
        $data = array();
        $data['body'] = "拼团商品";
        $data['out_trade_no'] = $order['ordernums'];
        $data['total_fee'] = abs(intval($pay_money));
        $data['spbill_create_ip'] = get_client_ip();
        $data['notify_url'] = 'https://houtai.rzmwzc.com/wap/ptapi/ptweixinhuiidoa';
        $data['trade_type'] = 'JSAPI';
        $data['openid'] = !empty($_REQUEST['openid'])?$_REQUEST['openid']:$member['xcxopenid'];
    
        $wechat = new WechatAction();

        $url = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
        $res = $wechat->open($data,$url);

        echo json_encode($res);

    }

    public function ptweixinhuiidoa(){

        echo "SESSION";
    }

    public function qxorder(){
        $out_trade_no = $_REQUEST['ordernums'];//out_trade_no
        $uid = $_REQUEST['uid'];

        //当前订单
        $order_num = M('pt_order')->where(array('ordernums'=>$out_trade_no))->find();
        ...
        $datae["status"]=8;
        $res=M("pt_order")->where(array('ordernums'=>$out_trade_no))->save($datae);
        if($res===false){
            $mes['status'] = 100;
            $mes['msg'] = "操作失败";
            outJson($mes);
    ...
    public function dopay(){
        ...
        if(empty($order_num["tid"])){
            $goods = M('pt_goods')->where(array('id'=>$order_num['gid']))->find();
            $data["gid"]=$order_num['gid'];//$gid;
            $data["uid"]=$uid;
            $data["title"]=$goods["title"];
            $data["type"]=1;//1购买团、2推广团
            $data["num"]=1;
            $data["money"]=$goods["yongjin"];//佣金
            $data["zengjia"]=$goods["zengjia"];//单人佣金
            $data["price"]=$goods["price"];
            $data["image"]=unserialize($goods['images'])[0]['img'];
            $data["time"]=time();
            $data["end_time"]=strtotime("+".$goods['shixiao']." hour",time());
            $data["cnum"]=$goods["renshu"];
            $data["tuan_type"]=$goods["tuan_type"];
            //$hongbao = M('pt_hongbao')->where(array('uid'=>$order_num['uid'],'tid'=>$order_num['tid']))->find();
            $res=M("pt_tuan")->add($data);
        ...
        //扣除佣金支付部分
        if($order_num['pay_yongjin']>0){
        //   $memberMoneyLog = 
            memberMoneyLog($order_num['uid'], 304, $order_num['pay_yongjin'], $info = "扣除佣金支付部分", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
            // var_dump($memberMoneyLog);exit;
        }

        //红包使用
        if($order_num['hbid']>0){
            M('pt_hongbao')->where(array('id'=>$order_num['hbid']))->save(array('type'=>1));
        }
        //团人数加一
        $pt_tuan=M("pt_tuan");
        $pt_tuan->startTrans();
        $pt_tuan->where("id=".$order_num['tid'])->setInc('num',1);
        $pt_tuan->commit();

        //购买的商品
        $goods = M('pt_goods')->where(array('id'=>$order_num['gid']))->find();
        //商品库存减少      下单已扣除
        // M('pt_goods')->startTrans();
        // M('pt_goods')->where(array('id'=>$order_num['gid']))->setDec('kucun',$order_num['num']);
        // M('pt_goods')->commit();
        //当前团信息
        $tuan = M('pt_tuan')->where(array('id'=>$order_num['tid']))->find();
        //当前订单数量
        $order_nums = M('pt_order')->where(array('uid'=>$order_num['uid']))->count();
        //当前订单会员信息
        $member = M('members')->where(array('id'=>$order_num['uid']))->find();

        //超级团
        if($tuan['tuan_type'] == 1){
            //会员第一次购买 会员上级获得邀请奖励
            if($order_nums<=1 && $member['recommend_id']!=0){
                M('pt_stop')->add(array(
                    'tuid'=>$member['recommend_id'],//受益人
                    'uid'=>$order_num['uid'],
                    'tid'=>$order_num['tid'],
                    'oid'=>$order_num['id'],
                    'num'=>$goods['zengjia'],
            ...
            //推荐人获得下级购买后红包奖励
            if($order_num['hbid']!=0){
                $hongbao = M('pt_hongbao')->where(array('id'=>$order_num['hbid']))->find();
                M('pt_stop')->add(array(
                    'tuid'=>$tuan['uid'],
                    'uid'=>$order_num['uid'],
                    'tid'=>$order_num['tid'],
        ...
        //判断是否成功成团
        if($tuan["cnum"]<$tuan["num"]){
            // M('pt_tuan')->where("id=".$tuan['id'])->setField('status','2');
            M('pt_stop')->where("tid=".$tuan['id'],'status=0')->save(array('status'=>1,'jd_time'=>time()));
            if($tuan['tuan_type']==1){
                memberMoneyLog($member['recommend_id'], 303, $goods['zengjia'], $info = "邀请新人奖励", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
                if($order_num['hbid']!=0){
                    memberMoneyLog($tuan['uid'], 303, $hongbao['hongbao'], $info = "下级购买红包奖励", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
                }
            }
        }
        ...
        if($tuan["cnum"]==$tuan["num"]){
            ...
            // $xin= M('pt_tuan')->where("id=".$tuan['id'],"type=1")->select();
            //成团解冻奖励
            $xin= M('pt_stop')->where("tid=".$tuan['id'],"status=0")->select();
            foreach($xin as $k=>$v){
                memberMoneyLog($v['tuid'], 303, $v['num'], $v['content'], $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
            }
            //修改奖励状态
            M('pt_stop')->where("tid=".$tuan['id'],'status=0')->save(array('status'=>1,'jd_time'=>time()));
            ...
            memberMoneyLog($tuan['uid'], 303, $goods['yongjin'], "成团获得佣金！", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);

            $members = M('members')->where(array('id'=>$tuan['uid']))->find();
            $up_status = M('members_status')->where(array('uid'=>$members['recommend_id']))->getField('partner');
            $upup_status = M('members_status')->where(array('uid'=>$members['recommend_bid']))->getField('partner');
            if($up_status==1){
                $up_status_add = M('pt_stop')->add(array(
                    'tuid'=>$members['recommend_id'],
                    'uid'=>$order_num['uid'],
                    'tid'=>$order_num['tid'],
                    'oid'=>$order_num['id'],
                    'num'=>$goods['yongjin']*0.2,
                    'status'=>1,
                    'add_time'=>time(),
                    'type'=>0,
                    'hide'=>1,
                ...
                if($up_status_add){
                    memberMoneyLog($member['recommend_id'], 303, $goods['yongjin']*0.2, "下级成团获得佣金！", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
                }
            }

            if($upup_status==1){
                $upup_status_add = M('pt_stop')->add(array(
                    'tuid'=>$members['recommend_id'],
                    'uid'=>$order_num['uid'],
                    'tid'=>$order_num['tid'],
                    'oid'=>$order_num['id'],
                    'num'=>$goods['yongjin']*0.1,
                    'status'=>1,
                    'add_time'=>time(),
                    'type'=>0,
                    'hide'=>1,
                ...
                if($upup_status_add){
                    memberMoneyLog($member['recommend_bid'], 303, $goods['yongjin']*0.1, "下下级成团获得佣金！", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
                }
            }
        }
        ...
        //判断成为合伙人
        $tuandui = M('pt_tuan')->where(array('uid'=>$tuan['uid'],'status'=>2))->count();
        if($tuandui==3){
            M('members_status')->where(array('uid'=>$tuan['uid']))->setField('partner',1);
        }
    ...
    public function audit()
    {
        ...
        $member = M('members_status')->where(array('uid'=>$uid))->find();

        $find = M('pt_audit')->where(array('uid'=>$uid,'status'=>0))->order('id desc')->find();
        if(!empty($find)&&$member['partner']!=1){
            $jsons['msg']="请勿重复提交申请";
            $jsons['status']=0;
            outJson($jsons);
        }else{
            ...
            $data['time'] = time();
            $aud_add =  M('pt_audit')->add($data);
            if($aud_add!==false){
                $jsons['msg']="申请已发出，请耐心等待审核";
                $jsons['status']=1;
                outJson($jsons);
            }else{
                $jsons['msg']="申请发送失败";
    ...
    public function paylogin(){
        ...
        require __DIR__.'/test/AopClient.php';
        require __DIR__.'/test/request/AlipaySystemOauthTokenRequest.php';

        $code = $_REQUEST['authcode'];
        
        $aop = new \AopClient();
        $aop->gatewayUrl = 'https://openapi.alipay.com/gateway.do';
        $aop->appId = '2021001164633603';//your app_id
        $aop->rsaPrivateKey = '';//请填写开发者私钥去头去尾去回车，一行字符串
        $aop->alipayrsaPublicKey='';//请填写支付宝公钥，一行字符串
        $aop->apiVersion = '1.0';
        $aop->signType = 'RSA2';
        ...
        if(!empty($resultCode)&&$resultCode != 10000){
            ...
            $test_user = M('members')->where(array('alipay_user_id'=>$user_id))->find();
            if(empty($test_user)){
                $msg['status'] = 999;
                $msg['msg'] = "请绑定手机号";
                echo json_encode($msg);exit;
                ...
                $msg['status'] = 0;
                $msg['msg'] = "登陆成功";
                $msg['uid'] = $this->jwt->getToken($test_user['id']);
                echo json_encode($msg);exit;
            }
     ...
     * 支付宝绑定手机号
     */
    public function payuser_phone(){
        $code = $_REQUEST['response'];

        $data = $this->paydecryptData($code);

        echo $data;
    }

    private function paydecryptData( $encryptedData )
    {
        $key = '8kbHZ4FaI4D3CxGgUG3R4Q==';//支付宝生成的aesKey
     ...
     * 微信登录
     */
    public function cxlogin()
    {
        $appid = 'wx32c335c0c20608c8';//appid
        $secret = 'f3829f6218f570d073899ea0baf8c37f';//secret
        ...
        // var_dump($res);exit;
        if($res['errcode']==0){
            $member = M('members')->where(array('xcxopenid'=>$res['openid']))->find();
            if(!empty($member)){
                M('members')->where(array('xcxopenid'=>$res['openid']))->save(array('session_key'=>$res['session_key']));
                M('member_info')->save(array('uid'=>$member['id'],'user_img'=>$user_img));//,'nick_name'=>$nick_name
                $jsons['msg']="登录成功";
                $jsons['status']=$res['errcode'];
                $jsons['uid']=$this->jwt->getToken(text($member['id']));
                ...
                if(!empty($res['unionid'])){
                    //根据unionid绑定用户
                    $find = M('members')->where(array('unionid'=>$res['unionid']))->find();
                    if(!empty($find)){
                        M('members')->where(array('unionid'=>$res['unionid']))->save(array('xcxopenid'=>$res['openid'],'session_key'=>$res['session_key']));
                        $jsons['msg']="登录成功";
                        $jsons['status']=$res['errcode'];
        //               $jsons['info']=array('session_key'=>$res['session_key'],'uid'=>$find['id']);
                        $jsons['uid']=$this->jwt->getToken(text($find['id']));;
                        $jsons['openid']=$res['openid'];
                        outJson($jsons);
                    }
                }
    ...
    public function phone_user()
    {
        ...
        $res = json_decode($_POST['info'],true);//$_POST['info'];
        $user_img = $_REQUEST['user_img'];
        $nick_name = $_REQUEST['nick_name'];
        $upuid = $this->jwt->verifyToken($_REQUEST["upuid"]);//$_REQUEST['upuid'];
        
        $iv = $this->decryptData($appid,$res['session_key'],$_POST['encrypteddata'],$_POST['iv'], $data);

        $ivdata = json_decode($data,true);

        // var_dump(json_decode($data,true));exit;
        if($iv==0){
            $phone = $ivdata['phoneNumber'];
        }else{
            $jsons['msg']="手机号获取失败";
            $jsons['status']=1000;
            outJson($jsons);
        ...
        if(isset($res['unionid'])){
            $phone_info = M('member_info')->where(array('cell_phone'=>$phone))->find();
            $member = M('members')->where(array('id'=>$phone_info['uid']))->find();
            if(!empty($phone_info)&&empty($member['xcxopenid'])&&empty($member['unionid'])){
                M('members')->where(array('id'=>$phone_info['uid']))->save(array('xcxopenid'=>$res['openid'],'session_key'=>$res['session_key']));
                $jsons['msg']="登录成功！";
                $jsons['status']=0;
                $jsons['uid']=$this->jwt->getToken(text($phone_info['uid']));//$phone_info['uid'];
                $jsons['openid']=$res['openid'];
                outJson($jsons);
            }
            ...
            //绑定手机号
            if(empty($phone_info)){
                $bind = M('members')->where(array('xcxopenid'=>$res['openid']))->find();
                $test_info = M('member_info')->where(array('uid'=>$bind['id']))->getField('cell_phone');
                if(!empty($bind)&&empty($test_info)){
                    M('member_info')->add(array(
                        'uid'=>$bind['id'],
                        'cell_phone'=>$phone
                    ));
                    // $sql['user_phone'] = $phone;
                    // M('members')->where("id={$bind['id']}")->save($sql);

                    $jsons['msg']="绑定成功";
                    $jsons['status']=0;
                    $jsons['uid']=$this->jwt->getToken(text($bind['id']));//$bind['id'];
                    $jsons['openid']=$res['openid'];
                    outJson($jsons);
                }
            }
            ...
            //根据unionid绑定用户
            $finds = M('members')->where(array('unionid'=>$res['unionid']))->find();
            $members = M('members')->where(array('xcxopenid'=>$phone_info['openid']))->find();
            if(empty($finds)&&empty($members)){//&&empty($phone_info)
                //创建新用户

                $dataa['user_name'] = $phone;
                $dataa['user_pass'] = trim(md5("123456"));
                $dataa['user_phone'] =$phone;
                

                $dataa['xcxopenid'] = $res['openid'];
                $dataa['unionid'] = $res['unionid'];
                // $dataa['session_key'] = $res['session_key'];
                $dataa['reg_time'] = time();
                $dataa['reg_ip'] = get_client_ip();
                $dataa['lastlog_time'] = time();
                $dataa['lastlog_ip'] = get_client_ip();
                $dataa['incode'] = getincode();

                $newid = M('members')->add($dataa);
                if(!empty($upuid)||$upuid!=0){
                    $this->bindup($newid,$upuid);
                }
                ...
                if(!$vo){
                    ...
                    M('member_money')->add(array('uid'=>$newid));
                    ...
                    if(empty($phone_info)){
                        ...
                        $sql['user_phone'] = $phone;
                        M('members')->where("id={$newid}")->save($sql);
                    }
                ...
                $jsons['msg']="登录成功";
                $jsons['status']=0;
                $jsons['uid']=$this->jwt->getToken(text($newid));;
                $jsons['openid']=$res['openid'];
                outJson($jsons);
            }else{
                $jsons['msg']='绑定失败';
    ...
    public function union()
    {
        ...
        $access_token = $this->curl("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=$appid&secret=$secret");
         //var_dump($access_token);
        $i=0;$j=0;
        if(!empty($access_token['access_token'])){
            //查找所有的unionID
            $data = M('members')->where("openid is not null and openid!='' and (unionid is null or unionid='')")->select();
            $i=count($data);
            //->where(array('openid'=>array('neq',''),'unionid'=>array('is','null')))
            //->select();
              // var_dump(M('members')->getlastsql());
              // var_dump($data);exit;
            foreach($data as $k=>$v){
                $url = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$access_token['access_token'].'&openid='.$v['openid'].'&lang=zh_CN';
                
                $res = $this->curl($url);
                
                if(isset($res['unionid'])){
                    $j=$j+1;
                    M('members')->where(array('id'=>$v['id']))->save(array('unionid'=>$res['unionid']));
                }
                var_dump($j);
            }
        }else{
    ...
    public function bindup($uid,$data)
    {
        // $data = $_REQUEST['upid'];//上级id
        // $uid = $_REQUEST['uid'];

        $openId = $data;

        $find = M('members')->where(array('id'=>$uid))->find();
        if($data!=$uid){
            if(date('Ymd',$find['reg_time'])==date('Ymd')&&$find['recommend_id']==0&&$find['recommend_bid']==0){
                $up = M('members')->where(array('id'=>$openId))->find();

                $sql['recommend_id'] = $up['id'];
                $sql['recommend_bid'] = $up['recommend_id'];
                $sql['recommend_cid'] = $up['recommend_bid'];

                M('members')->where(array('id'=>$uid))->save($sql);
            }
        }
    }
    ...
    {
        $name = $this->replace_specialChar($_REQUEST['name']);
        $type = $_REQUEST['type'];
        $page = $_REQUEST['page'];
        $size = $_REQUEST['size'];
        // var_dump($page-1);exit;
        ...
        // var_dump($limit);exit;
        if(!empty($type)){
        //    $map['leixing'] = array('like','%'.$type.'%');
        $map['typeid'] = $type;
        }
        $map['online'] = 1;
        $goods = M('pt_goods')->where($map)
        ->limit("$limit,$size")
        ->select();

        foreach ($goods as $key => $value) {
            $str = strlen($value['leixing'])-1;
            $len = substr($value['leixing'],1,$str);
            $ex = explode('|',$len);
            if($ex!=false){
                $where['id'] = array('in',$ex);
            }else{
                $where['id'] = $ex;
            }
            $goods[$key]['images'] = unserialize($value['images']);
            $goods[$key]['table'] = M('pt_type')->where($where)->select();

        }

        $return = empty($goods)?'':json_encode($goods);
    ...
    {
        $data = M('pt_type')->where(" type=1 ")->select();
    ...
    public function tuijian()
    {
        ...
        switch ($type) {
            case 1: //推荐
                $data = M('ad')->where(array('id'=>45))->find();
                $data['content'] = unserialize($data['content']);
                echo json_encode($data);exit;
                break;
            case 2: //热卖
                // $map['leixing'] = array('like','%'. 5 .'%');
                $map['typeid'] = 5;
            break;
            case 3: //每日好货
                // $map['leixing'] = array('like','%'. 3 .'%');
                $map['typeid'] = 3;
            break;

            default:
                // $map['leixing'] = array('like','%'. 1 .'%');
                $map['typeid'] = 1;
            break;
        }
        ...
        $map['online'] = 1;
        $goods = M('pt_goods')->where($map)->limit($limit,$size)->select();
    ...
    {
        $map['id'] = $_REQUEST['gid'];
        $tid['tid'] = $_REQUEST['tid'];
        $size = $_REQUEST['size'];
        $page = ($_REQUEST['page']-1)*$size;

        $goods = M('pt_goods')->where($map)->find();
        
        // var_dump($_POST['gid']);exit;
        
        $goods['images'] = unserialize($goods['images']);

        $tuan = M('pt_tuan t')->where(array('t.gid'=>$map['id'],'t.status'=>1,'t.end_time'=>array("gt",time())))
        // ->join('lzh_members m ON m.id = t.uid')
        ->join('lzh_member_info i ON i.uid = t.uid')->join('lzh_members m ON m.id = t.uid')
        ->field('t.id,i.user_img,i.nick_name,t.cnum,t.num,t.end_time,m.user_name')
        ->limit("$page,$size")
        ->select();
        foreach ($tuan as $k=>$v){
            if($v["nick_name"]==''){
                $tuan[$k]["nick_name"]=$v["user_name"];
            }
        }
        $count = M('pt_tuan t')->where(array('t.gid'=>$map['id'],'t.status'=>1))->count();
       
        $order = M('pt_order')->where(array('tid'=>$tid))->select();

        outJson(array('goods'=>$goods,'tuan'=>$tuan,'order'=>$order,'sdate'=>time(),'count'=>$count));
    }
    ...
    {
        $uid = $_REQUEST['uid'];
        $addr = M('member_address')->where(array('uid'=>$uid))->select();
        outJson($addr);
    }
    ...
    public function moren_address(){
        ...
        $sa['default'] = 0;
        $sa2['default'] = 1;
        $info = M('member_address')->where($data)->save($sa);
        $info2 = M('member_address')->where($data2)->save($sa2);
        if($info && $info2){
            $retu['stu']=1;
            $retu['msg']="设置默认地址成功";
    ...
    */
    ...
    public function dobanksave()
    {
        ...
        if($data['is_default']==1){
            ...
            $newid = M('member_banks')->where("uid = ".$uid)->save(["is_default"=>0]);
    ...
    public function bankselect(){
        ...
        $map['id'] = $_REQUEST['id'];
        $bank = M("member_banks")->where($map)->find();//"id=".$_REQUEST['id'].",uid=".$uid 
    ...
    public function withdrawal()
    {
        ...
        $bank = M('member_banks')->where(array('id'=>$bid,'uid'=>$uid))->find();
        if(empty($bank)){
            $msg['status'] = 1;
            $msg['msg'] = "请绑定银行卡";
            outJson($msg);
        ...
        $member = M('member_money')->where(array('uid'=>$uid))->find();

        if($money>$member['yongjin']){
            $msg['status'] = 1;
            $msg['msg'] = "可提现佣金不足";
            outJson($msg);
        }

       $dec =  memberMoneyLog($uid, 304, $money, $info = "扣除申请提现金额", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
    ...
    public function hongbao()
    {
        ...
        $verfy = M('pt_hongbao')->where(array('uid'=>$uid,'tid'=>$tid))->find();
        if(empty($verfy)){

            $tuan = M('pt_tuan')->where(array('id'=>$tid))->find();
            if(empty($tuan)){
                $msg['status'] = 300;
                $msg['msg'] = '拼团不存在';
                outJson($msg);
            ...
            $sql['uid'] = $uid;
            $sql['tid'] = $tid;
            $sql['hongbao'] = rand(1,2)/100;
            $sql['add_time'] = time();

            $data = M('pt_hongbao')->add($sql);
            if($data!==false){
                $msg['status'] = 200;
                $msg['msg'] = '恭喜获得'.$sql['hongbao']."元红包";
                outJson($msg);
            }else{
                $msg['status'] = 500;
                $msg['msg'] = '领取失败';
    ...
    //     $limit = $page * $size;
    //     $type['type'] = array('in',array(301,302,303));

    //     $data = M("member_moneylog")->where(array('uid'=>$uid))
    //         ->where($type)
    //         ->limit($limit,$size)->select();
    ...
    public function member()
    {
        ...
        $field = 'm.id,i.nick_name,s.partner,i.user_img,mm.yongjin,m.user_phone';//mm.yongjin

        $data = M("members m")
            ->join('lzh_members_status s ON m.id = s.uid')
            ->join('lzh_member_info i ON m.id = i.uid')
            ->join('lzh_member_money mm ON m.id = mm.uid')
            ->field($field)
            ->where(array('m.id'=>$uid))
            ->find();

        if($data['partner'] != 1){
            $data['partner_count'] = M('pt_tuan')->where(array('uid'=>$uid,'status'=>2))->count();
        }else{
            $data['partner_count'] = 3;
        }
        $day1 = strtotime(date('Ymd'));
        $day2 = $day1+(24*60*60);

        $m1 = strtotime(date('Ym'));
        $m2 = $day1+(30*24*60*60);
        $data['day'] = M('pt_stop')->where(array('tuid'=>$uid,'status'=>1,'jd_time'=>array('BETWEEN',array($day1,$day2))))->sum('num');
        $data['mom'] = M('pt_stop')->where(array('tuid'=>$uid,'status'=>1,'jd_time'=>array('BETWEEN',array($m1,$m2))))->sum('num');

        $tuan['dai'] = count(M('pt_tuan')->where(array('uid'=>$uid,'status'=>1))->select());
        $tuan['cheng'] = count(M('pt_tuan')->where(array('uid'=>$uid,'status'=>2))->select());
        $tuan['bai'] = count(M('pt_tuan')->where(array('uid'=>$uid,'status'=>3))->select());

        $order['daifukuan'] = count(M('pt_order')->where(array('uid'=>$uid,'status'=>0))->select());
        $order['daifahuo'] = count(M('pt_order')->where(array('uid'=>$uid,'status'=>1))->select());
        $order['daishouhuo'] = count(M('pt_order')->where(array('uid'=>$uid,'status'=>2))->select());
        $order['daipingjia'] = count(M('pt_order')->where(array('uid'=>$uid,'status'=>3))->select());
        $order['daituikuan'] = count(M('pt_order')->where(array('uid'=>$uid,'status'=>7))->select());

        $kefu = '11012011910086';
        outJson(array('member'=>$data,'tuan'=>$tuan,'order'=>$order,'kefu'=>$kefu));
    }
    ...
    public function teamhead(){
        ...
        $field = 'm.reg_time,m.id,i.nick_name,i.user_img';

        

        $user = M('members m')->where(array('m.id'=>$uid))
        ->join('lzh_member_info i ON m.id = i.uid')
        ->field($field)
        ->find();
        

        $up = M('members m')->where(array('m.id'=>$user['recommend_id']))
        ->join('lzh_member_info i ON m.id = i.uid')
        ->field($field)
        ->find();

        $shouyi = M('pt_stop')->where(array('tuid'=>$uid,'status'=>1))->sum('num');

        $datawhere = array('recommend_id'=>$uid);
        $teamwhere = array('recommend_bid'=>$uid);
        $data = M('members')->where($datawhere)->count();
        $team = M('members')->where($teamwhere)->count();
    ...
    public function team()
    {
        ...
        if($type==1){
            $datawhere['s.partner'] =1;
        }else if($type==2){
            $datawhere['s.partner'] =0;
        }

        if($status==1){
            $datawhere['m.recommend_id'] = $uid;
        }else{
            $datawhere['m.recommend_bid'] = $uid;
        }
        $field = 'i.user_img,m.user_name,m.reg_time,m.id,i.nick_name,s.partner';
        
        $data = M('members m')->where($datawhere)
            ->join('lzh_member_info i ON m.id = i.uid')
            ->join('lzh_members_status s ON m.id = s.uid')
            ->limit("$page,$size")
            ->field($field)
            ->select();
        foreach($data as $k=>$v){
            $data[$k]['yaoqing'] = M('members')->where(array('recommend_id'=>$v['id']))->count();
            $data[$k]['tuan'] = M('pt_tuan')->where(array('uid'=>$v['id'],'status'=>2))->count();
        }
    ...
    public function teamsousuo()
    {
        ...
        // $page = ($_REQUEST['page']-1)*$size;
        if(!empty($name)){
            $datawhere['i.nick_name'] = $name;
            // $teamwhere = array('i.nick_name'=>$name);
        }
        
        $field = 'i.user_img,m.user_name,m.reg_time,m.id,i.nick_name';

        $datawhere['m.recommend_id|m.recommend_bid'] = $uid;
        $data = M('members m')->where($datawhere)
            ->join('lzh_member_info i ON m.id = i.uid')
            ->join('lzh_members_status s ON m.id = s.uid')
            // ->limit("$page,$size")
        ...
        // $teamwhere = array('recommend_bid'=>$uid);
        // $data = M('members m')->where($teamwhere)
        //     ->join('lzh_member_info i ON m.id = i.uid')
        //     ->join('lzh_members_status s ON m.id = s.uid')
        //     ->limit("$page,$size")
        //     ->field($field)
    ...
    public function shourumingxi(){
        ...
        foreach($data as $k=>$v){
            $gid = M('pt_order')->where(array('id'=>$v['oid']))->getField('gid');
            $data[$k]['goods'] = M('pt_goods')->where(array('id'=>$gid))->find();
        }

        $sssdj = M('pt_stop')->where(array('tuid'=>$uid,'status'=>0))->sum('num');
        $sssjd = M('pt_stop')->where(array('tuid'=>$uid,'status'=>1))->sum('num');
        $ssssx = M('pt_stop')->where(array('tuid'=>$uid,'status'=>2))->sum('num');
        $stop['dj'] = empty($sssdj)?0:$sssdj;
        $stop['jd'] = empty($sssjd)?0:$sssjd;
        $stop['sx'] = empty($ssssx)?0:$ssssx;
        $jb = M('pt_tuan')->where(array('uid'=>$uid,'status'=>1))->count();
        $stop['jb'] = $jb>3?3:$jb;
        
        echo json_encode(array('data'=>$data,'stop'=>$stop));
    }
    ...
    public function myorders()
    {
        ...
        $page = ($_REQUEST['page']-1)*$size;
//        $size = $_POST['size'];
//        $page = $_POST['page']*$size;
//    
        $status = $_POST['status'];
       // var_dump(1);
        //自动收货
        $this->autoordertodos($uid);
       // var_dump(2);
        //自动退货
        $tuan_s= M('pt_order')->where(array('uid'=>$uid))->select();
        foreach($tuan_s as $kt=>$vt){
            $this->autotuikuan($vt['tid']);
        }
      //  var_dump(3);
        if($status==0){
            $status = array('in',array(0,1,2,3,4,5,6,7));
        }else if($status==7){
            $status = 7;
        }else{
            $status = $status-1;
        ...
        $map = array('o.uid'=>$uid,'o.status'=>$status);

        $field = 'g.title,g.images,g.shorttitle,g.renshu,o.id,o.money,o.add_time,o.type,o.status,o.ordernums,o.uid,o.pingjia,o.gid,o.kuaidi,o.wuliu';
        $data = M('pt_order o')
            ->join('lzh_pt_goods g ON o.gid = g.id')
            ->field($field)
            ->where($map)
            ->limit("$page,$size")
            ->order('add_time desc')
        ...
        foreach ($data as $key => $value) {
            ...
            $data[$key]['tuikuan'] = M('pt_refund')->where(array('oid'=>$value['id']))->getField('type');
            $data[$key]['refunds'] = M('pt_refunds')->where(array('oid'=>$value['id']))->getField('type');
     ...
     * 退货地址
     */
    public function refundsaddress(){
        $oid = $_REQUEST['oid'];
        $uid = $_REQUEST['uid'];

        $data = M('pt_refund')->where(array('oid'=>$oid['id'],'uid'=>$uid))->find();
    ...
    public function orderdetails()
    {
        ...
        $data = M('pt_order o')->where(array('uid'=>$uid,'id'=>$id))->find();

        $hongbao = M('pt_hongbao')->where(array('id'=>$data['hbid']))->find();
        $hongbao = $hongbao?$hongbao:array();

        $goods = M('pt_goods')->where(array('id'=>$data['gid']))->find();
        $goods['images'] = unserialize($goods['images']);

        $tuan = M('pt_tuan')->where(array('id'=>$data['tid']))->find();

        $data['images'] = unserialize($data['images']);


        outJson(array('order'=>$data,'goods'=>$goods,'tuan'=>$tuan,'hongbao'=>$hongbao));
    }
    ...
    public function queren(){
        ...
        $find = M('pt_order o')
            ->field('o.tid,t.num,t.cnum,t.money,t.zengjia,t.hnum,t.tuan_type')
            ->join('lzh_pt_tuan t ON o.tid = t.id')
            ->where(array('o.uid'=>$uid,'o.id'=>$id,'status'=>2))
            ->find();
        if(empty($find)){
            $mes['status'] = 400;
            $mes['msg'] = '订单不存在';
        ...
        //确认收货
        $data = M('pt_order o')
            ->where(array('o.uid'=>$uid,'o.id'=>$id,'o.status'=>2))
            ->save(array('o.status'=>3));
     ...
     * 自动确认收货
     */
    public function autoordertodos($uid){

        //确认收货
        $data = M('pt_order o')
            ->where(array('o.uid'=>$uid,'o.status'=>2,'o.add_time'=>array('elt',time()-(15*24*60*60))))
            ->save(array('o.status'=>3));
        //var_dump(M('pt_order o')->getLastSql());
        if($data!==false) {
            $mes['status'] = 200;
            $mes['msg'] = '确认成功';
        }else{
     ...
     * 自动退款
     */
    private function autotuikuan($tid){
        /**
         * 下单调用
         * 团列表调用
         * 订单列表
        ...
        $time = time();
        M('pt_tuan')->startTrans();
        $tuan = M('pt_tuan')->where(array('id'=>$tid))->find();
        if($tuan['status']==1 && $tuan['end_time']<$time){

            include './WechattestAction.class.php';
            $wechat = new WechattestAction();

            $order = M('pt_order')->where(array('tid'=>$tid,'status'=>1))->select();
//            var_dump($tid);
//            var_dump($order);exit;
            foreach($order as $k=>$v){
                //var_dump("123");
                if($v['status']==1){
                    $hongbao = M('pt_hongbao')->where(array('id'=>$v['hbid']))->getField('hongbao');
                    $data['out_trade_no'] = $v['ordernums'];
                    $data['out_refund_no'] = $v['ordernums'];
                    $data['total_fee'] = intval(($v['money'] + $v['yunfei'] - $hongbao - $v['pay_yongjin'])*100);//支付金额
                    $data['refund_fee'] = intval(($v['money'] - $hongbao - $v['pay_yongjin'])*100);

                    if($v['zftype']!=1){
                        $aa=$wechat->ttt($data);
                    }
                    if($v['pay_yongjin']>0){
                        memberMoneyLog($v['uid'], 303, $v['pay_yongjin'], "拼团失败退还购买佣金", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
                    }
                    ...
                    // $sql['shuoming'] = '';
                    // $sql['imgs'] = $imgs;
                    $sql['ostatus'] = $v['status'];
                    $sql['type'] = 1;

                    $pt_refund = M('pt_refund')->add($sql);
                    ...
                    $data = M('pt_refunds')->add($sqll);
                }
            }

            M('pt_order')->where(array('tid'=>$tid))->save(array('status'=>7));
            M('pt_tuan')->where(array('id'=>$tid))->save(array('status'=>3));
            M('pt_stop')->where(array('tid'=>$tid))->save(array('status'=>2));
        }
        M('pt_tuan')->commit();
        
    }

    public function refundscause($id){
        include './WechattestAction.class.php';
        //$id = $_REQUEST['id'];
        $order = M('pt_order')->where(array('id'=>$id))->find();
        ...
        if($status==1){
            ...
            foreach($stop as $k=>$v){
                memberMoneyLog($v['tuid'], 304, $v['num'], $info = "团员退货扣除奖励", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
            }
            //因为此人的 改成失效的
        ...
        if($order['pay_yongjin']>0){
            memberMoneyLog($order['uid'], 303, $order['pay_yongjin'], $info = "退货退还佣金支付部分", $target_uid = 0, $target_uname = "",$shiwu=true,$yubi=0);
        }
        if($order['zftytpe']!=1){
            //退还微信支付部分
            $data['out_trade_no'] = $order['ordernums'];
...
//        }
    ...
    public function evaluation()
    {
        ...
        $gid = M('pt_order')->where(array('id'=>$oid))->getField('gid');

        $sql['uid'] = $uid;
        $sql['gid'] = $gid;
        $sql['oid'] = $oid;

        $sql['product'] = $_REQUEST['product'];
        $sql['service'] = $_REQUEST['service'];
        $sql['delivery'] = $_REQUEST['delivery'];
        $sql['content'] = text($_REQUEST['content']);

        $sql['add_time'] = time();

        $find = M('pt_evaluation')->where(array('uid'=>$uid,'oid'=>$oid,'gid'=>$gid))->find();
        ...
        $add = M('pt_evaluation')->add($sql);
        $ping['pingjia'] = 1;
        // $save = M('pt_order')->where(array('id'=>$oid))->save($ping);
        $save = M()->query("UPDATE `lzh_pt_order` SET `pingjia`=1 WHERE `id`=$oid");
        // var_dump($save);exit;
    ...
    public function tuikuan()
    {
        ...
        $order = M('pt_order')->where(array('uid'=>$uid,'id'=>$oid))->find();
        ...
        $find = M('pt_refund')->where(array('uid'=>$uid,'oid'=>$oid))->find();
        if(!empty($find)){
            $mes['status'] = 100;
            $mes['msg'] = '您已提交，请耐心等待';
            outJson($mes);
        ...
        $sql['add_time'] = time();
        $sql['cause'] = $content;
        $sql['shuoming'] = $shuoming;
        $sql['ostatus'] = $order['status'];
        $sql['imgs'] = $imgs;

        $add = M('pt_refund')->add($sql);

        $save = M('pt_order')->where(array('uid'=>$uid,'id'=>$oid))->save(array('status'=>7,'reason'=>$content));
    ...
    public function jintuikuan(){
        ...
        if($add!==false &&$save!==false &&$data!==false){
            $mes['status']=200;
            $mes['msg'] = "提交成功";
            outJson($mes);
        }else{
    ...
    public function savetuikuan()
    {
        ...
        $add = M('pt_refund')->where(array('id'=>$id))->save($sql);

        $save = M('pt_order')->where(array('uid'=>$uid,'id'=>$oid))->save(array('status'=>7,'reason'=>$content));

        if($save!==false&&$add!==false){
            $mes['status'] = 200;
            $mes['msg'] = '修改退款成功，请耐心等待';
        }else{
            $mes['status'] = 500;
            $mes['msg'] = '修改退款失败';
        }
     ...
     * 退款详情
     */
     public function tuikandetail(){
         $uid = $_REQUEST['uid'];
         $oid = $_REQUEST['oid'];

         $data = M('pt_order')->where(array('uid'=>$uid,'id'=>$oid))->find();
         ...
         $tuikuan = M('pt_refund')->where(array('uid'=>$uid,'oid'=>$oid))->find();
         $refunds = M('pt_refunds')->where(array('uid'=>$uid,'oid'=>$oid))->find();

         $goods = M('pt_goods')->where(array('id'=>$data['gid']))->find();
         $goods['images'] = unserialize($goods['images']);

         outJson(array('order'=>$data,'tuikuan'=>$tuikuan,'goods'=>$goods,'hongbao'=>$hongbao,'refunds'=>$refunds));
     }

     /**
      * 退款撤销
      */
      public function deltuikuan(){
        $id = $_REQUEST['id'];
        $uid = $_REQUEST['uid'];

        $data =  M('pt_refund')->where(array('id'=>$id,'uid'=>$uid))->find();

        $order = M('pt_order')->where(array('id'=>$data['oid'],'uid'=>$uid))->find();

        $refunds =  M('pt_refunds')->where(array('oid'=>$data['oid'],'uid'=>$uid))->find();

        if($order!==false&&$data!==false&&$refunds['type']!=1){

            $resorder = M('pt_order')->where(array('id'=>$data['oid'],'uid'=>$uid))->save(array('status'=>$data['ostatus']));
            $res = M('pt_refund')->where(array('id'=>$id,'uid'=>$uid))->delete();

            if($resorder!==false&&$res!==false){
                $mes['status'] = 200;
                $mes['msg'] = '撤销成功';
                outJson($mes);
            }
      ...
      public function refunds(){
        ...
        $wuliu = $_REQUEST['wuliu'];

        $refund = M('pt_refund')->where(array('uid'=>$uid,'oid'=>$oid,'type'=>1))->find();
     ...
     * 热搜词添加
     */
    public function resc($name)
    {
       $test = M('pt_resc')->where(array('name'=>$name))->setInc('num',1);
       if($test==false){
           M('pt_resc')->add(array(
               'name'=>$name,
               'num'=>1,
    ...
    {
        $data = M('pt_resc')->order('num desc')->limit(10)->select();
    ...
    public function moneydetails()
    {
        ...
        $data = M('pt_tuan t')
                ->where($map)
                ->join('lzh_pt_goods g ON g.id = t.gid')
                ->select();

        outJson(array('data'=>$data));
    }
     ...
     */
     ...
     public function benyue(){
        // $status = 10;
        $size = $_REQUEST['size'];
        $page = ($_REQUEST['page']-1)*$size;
        $data = M('pt_goods')->limit("$page,$size")->select();
        foreach($data as $k=>$v){
            $data[$k]['images'] = unserialize($v['images']);
        }
     ...
     public function guanggao(){
        ...
        if($id==1){
            //商品广告 首页轮播图
            $data = M('ad')->where(array('id'=>46))->find();
            $data['content'] = unserialize($data['content']);
        }else if($id==2){
            //团长介绍
            $data = M('ad')->where(array('id'=>42))->find();
            $data['content'] = htmlspecialchars_decode($data['content']);
        }else if($id==3){
            //拼团玩法
            $data = M('ad')->where(array('id'=>43))->find();
            $data['content'] = htmlspecialchars_decode($data['content']);
        }else if($id==4){
            //合伙人协议
            $data = M('ad')->where(array('id'=>44))->find();
            $data['content'] = htmlspecialchars_decode($data['content']);
        }else if($id==5){
            //合伙人轮播图
            $data = M('ad')->where(array('id'=>47))->find();
            $data['content'] = unserialize($data['content']);
        }
     ...
     public function qrcode(){
        //配置APPID、APPSECRET
         $APPID = "wx32c335c0c20608c8";
         $APPSECRET = "f3829f6218f570d073899ea0baf8c37f";//"aa9a094a992fa341cbc1bf2c9163279a";
        $upid = is_numeric($_REQUEST['uid'])?$this->jwt->getToken(text($_REQUEST['uid'])):$_REQUEST['uid'];
        //获取access_token
        ...
        $param = json_encode(array("path"=>"pages/index/index?upid=$upid","width"=> 150));
    ...
    private function hehe($uid){
        ...
        if(!empty($borrowinvestor)){
            ...
            $invest_defail_id = M('member_moneylog')->add(['uid'=>$this->uid,'type'=>'9','affect_money'=>$receive_interest,'account_money'=>$money['account_money']+$receive_interest,'back_money'=>'0','collect_money'=>'0','freeze_money'=>'0','info'=>'体验金回款','add_time'=>$add_times,'add_ip'=>get_client_ip(),'target_uid'=>0,'target_uname'=>'','experience_money'=>$borrowinvestor['investor_capital']]);
            ...
            $invest_defail_id = M('member_money')->save(["uid"=>$money['uid'],"money_freeze"=>$money['money_freeze']-$borrowinvestor['investor_capital'],"money_collect"=>$money['money_collect'],"account_money"=>$money['account_money']+$receive_interest,"back_money"=>$money['back_money'],"credit_limit"=>$money['credit_limit'],"credit_cuse"=>$money['credit_cuse'],"borrow_vouch_limit"=>$money['borrow_vouch_limit'],"borrow_vouch_cuse"=>$money['borrow_vouch_cuse'],"invest_vouch_limit"=>$money['invest_vouch_limit'],"invest_vouch_cuse"=>$money['invest_vouch_cuse'],"money_experience"=>$money['money_experience'],"update_time"=>time()]);
    ...
    */
...

    {
       ...
       $jsons["banner"]  = is_array($ad) ? $ad : array();
        ...
        $tiYanJin=array("nianHua"=>"24.00%","tiYanJin"=>"8888","zhouQi"=>"1","id"=>"1");
        ...
        foreach ($list_type1["list"] as $k => $v) {

            $list_s[$k]["id"]=$v["id"];

            $list_s[$k]["borrow_name"]=$v["borrow_name"];

            $list_s[$k]["borrow_img"]=$this->urls.$v["borrow_img"];

            $list_s[$k]["borrow_money"]=$v["borrow_money"];

            $list_s[$k]["progress"]=$v["progress"];

            $list_s[$k]["start_time"]=$v["start_time"];

            $list_s[$k]["borrow_min"]=$v["borrow_min"];

            $list_s[$k]["borrow_duration"]=$v["borrow_duration"]; //项目期限

            $list_s[$k]["borrow_interest_rate"]=$v["borrow_interest_rate"];

            $list_s[$k]["borrow_status"]=$v["borrow_status"];

            //'start_time'] + ($vb['collect_day']*60*60*24)

            if($v["borrow_status"]==2){

                $list_s[$k]["shengyu_time"]=$v["start_time"]+$v['collect_day']*60*60*24;

            }

            if($v["borrow_status"]>2){

                $list_s[$k]["shengyu_time"]=0;

            }

            

            $list_s[$k]["borrow_uname"]= M('members')->where("id=".$v["borrow_uid"])->getField('user_name');

        }

       $jsons["borrowStatus"]=c('borrow.BORROW_STATUS');

       $jsons["borrowList"] = $list_s;
    ...
    */
    ...
    public function ispass(){
        $minfo=M("members")->where("id=".$this->uid)->find();
        if(empty($minfo['pin_pass'])){
            $jsons["msg"]="支付密码为空，请先设置支付密码！";
            $jsons["status"] = "0";
        }else{
            $jsons["status"] = "1";
        }
	...
	// 小程序端获取openID接口
	public function getxcxopenid(){
		$appid = 'wx32c335c0c20608c8';//appid
		$secret = 'f3829f6218f570d073899ea0baf8c37f';//'aa9a094a992fa341cbc1bf2c9163279a';//secret
		$js_code = $_REQUEST['js_code'];
		$url = "https://api.weixin.qq.com/sns/jscode2session?appid=$appid&secret=$secret&js_code=$js_code&grant_type=authorization_code";
		$res = $this->curl($url);
		outJson($res);
    ...
    public function weiixnhuidiao(){
        ...
        $res="https://".$_SERVER['SERVER_NAME']."/app.html#/usercenter/wx_login_result?".$res;
    ...
    public function GetOpenid()
    {
        ...
        if (!isset($_GET['code'])){
            ...
            $res="https://".$_SERVER['SERVER_NAME']."/app.html#/getOpenidPage?".$res;
    ...
    public function weixinregaction(){
            ...
            if(!empty($mev['openid'])&&$mev['openid']!=$data['openid']){

                $jsons['status'] = "0";

                $jsons['msg'] = "注册失败，用户名或者手机号已经有人使用";
    ...
    */
    ...
    public function reg(){
        ...
        switch ($action) {
            ...
            case 'phoneverify':
                ...
                $verifyRs = M('verify_code')->where("id = '{$codeId}' and content = {$txtCode}")->find();
    ...
    public function sendcode(){
        ...
        $typed=$datag["dx_type"];
        if($typed==1){
            Notice1(1,0,array('phone'=>$txtPhone,"code"=>$code));
        }else{
            sendsms($txtPhone, $content);
        }

        $addData = array('content'=>$code,'add_time'=>time(),'phone'=>$txtPhone);
   ...
   */
    ...
    */
    ...
    public function sign(){
        ...
        $addsign['cumulative_day']=M("member_sign")->where("uid={$uid}")->order("sign_time desc")
    ...
    public function yaoqing(){
        ...
        $value = "https://".$_SERVER['SERVER_NAME']."/app.html#/login?invite=".$incode; //二维码内容
...
//die; 
        ...
        $binfo = M('borrow_info')->field('borrow_uid,borrow_money,has_borrow,has_vouch,borrow_max,borrow_min,borrow_type,password,pause,max_limit,start_time,new_user_only,bespeak_able,bespeak_money,bespeak_days')->find($borrow_id);
...
//var_dump($minfo);
    ...
    //       return(0); die; echo "cURL Error #:" . $err5.die;
        ...
        return"添加签署者,原因：".$msg5.$code5['code'];die;
    ...
    public function youhuijun(){
        ...
        $list = M('member_bonus t1')->join('lzh_members t2 on t1.uid = t2.id')->field('t1.*,t2.user_name')->where($map)->order('t1.id desc,t1.end_time asc')->limit($Lsql)->select();      
...

    {
        ...
        $field = "i.*,i.add_time as invest_time,m.user_name as borrow_user,b.*";
    ...
    public function order_list(){
        ...
        $field= 'id,gid,jifen,uid,num,action,real_name,address,jine,add_time,ordernums,carid,pay_way';
        ...
        if($action==4){
           ...
           $map["action"]=array("in",array(0,1,2,3));
    ...
    public function orderinfo(){
        $map["id"]=$_REQUEST["id"];
        $info=M("order")->field('id,gid,jifen,uid,num,action,cell_phone,real_name,address,jine,add_time,ordernums,carid,yijian,beizhu')->where($map)->find();
        if(!$info){
            $jsons['msg'] = "数据错误！";
            $jsons['status'] = '0';
            outJson($jsons);
    ...
    public function zhongzhi_order(){
        ...
        //$zzxx=M("zhongzhi_xx")->where("id=".$carinfo["gid"])->              
    ...
    public function zhongzhi_info(){
        ...
        $order=M('order_zz')->field('id,gid,jifen,uid,num,cell_phone,status,real_name,address,jine,add_time,ordernums,yijian')->where("id='".$id."'")->find();
    ...
    public function pttixian(){
        ...
        $Lsql = $page*$size.','.$size;
        $list = m("pt_withdrawal")->where($map)->order("id DESC")->limit($Lsql)->select();
        if(count($list)==$size){
            $limits=($page+1)*$size.','.$size;
            $countlist =m("pt_withdrawal")->where($map)->order("id DESC")->limit($limits)->select();
            if($countlist>0){
                $jsons['data']["ispage"]=1;
            }else{
                $jsons['data']["ispage"]=0;
