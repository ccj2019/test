...
require dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'./../HeepayOpenssl.php';
...
class AopClient {
	...
	protected $heepaySdkVersion = "heepay-sdk-php-20220824";
	...
	public function getSignContent($params) {
		$this->echoDebug("产生的签名明文数据排序前=".json_encode($params));
		ksort($params);
        $this->echoDebug("产生的签名明文数据排序后=".json_encode($params));
		$stringToBeSigned = "";
		$i = 0;
		foreach ($params as $k => $v) {
			if (false === $this->checkEmpty($v) && "@" != substr($v, 0, 1)) {
				...
				//$this->echoDebug("k=".$k."===========v=".$v."===stringToBeSigned=".$stringToBeSigned);
				$i++;
			}
		}

		unset ($k, $v);
		$this->echoDebug("产生的签名明文数据=".$stringToBeSigned);
	...
	public function sign($data, $signType = "RSA2",$un) {
		...
		$this->echoDebug("产生的签名数据=".$sign."签名类型=".$signType."使用的签名私钥=".$priKey);
		return $sign;
	}
    ...
    * post json
    */
    protected function curlJson($url, $postFields = null) {
    		$ch = curl_init();
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		$sysParams["merch_id"] = $request->getMerchId();
        ...
        if ($this->checkEmpty($apiParams['biz_content'])) {
            ...
            throw new Exception(" api request Fail! The reason : 业务参数为空!");
        }

        if ($this->checkEmpty($this->heepayPublicKey)) {

            throw new Exception("汇付宝公钥为空");
        }
        ...
        $this->echoDebug("加密前的biz_content=".$apiParams['biz_content']."加密用的汇元公钥=".$this->heepayPublicKey."加密用的编码=".$this->postCharset);
        $enCryptContent = $this->rsaEncrypt($apiParams['biz_content'], $this->heepayPublicKey, $this->postCharset);
        //$apiParams['biz_content'] = $enCryptContent;
        $this->echoDebug("加密的biz_content=".$enCryptContent);
		...
		//发起HTTP请求
		try {
		    $this->echoDebug("请求的URL=".$this->gatewayUrl."请求的参数=".json_encode($sysParams));
			$resp = $this->curlJson($this->gatewayUrl, $sysParams);
			$this->echoDebug("响应的参数=".$resp);
		} catch (Exception $e) {
        ...
        if($respObject->code != 10000){
            $this->echoDebug("请求出现错误");
        }

        $this->echoDebug("解析后的跳转URL=".$signData->signSourceData->sign_url);
		$signURL=$signData->signSourceData->sign_url;

		// 验签
		//$this->checkResponseSign($request, $signData, $resp, $respObject);

        $respObject->data = $signData->signSourceData;
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		($res) or die('汇付宝RSA公钥错误。请检查公钥文件格式是否正确');
...
/** 
	...
	public function rsaEncrypt($data, $rsaPublicKeyPem, $charset) {
        ...
        //读取字符串
        $pubKey= $rsaPublicKeyPem;
        $res = "-----BEGIN PUBLIC KEY-----\n" .
            wordwrap($pubKey, 64, "\n", true) .
            "\n-----END PUBLIC KEY-----";

		($res) or die('汇付宝RSA公钥错误。请检查公钥格式是否正确'.$res);

        openssl_public_encrypt($data, $chrtext, $res);
	...
	public function rsaDecrypt($data, $rsaPrivateKeyPem, $charset) {
        ...
        $this->echoDebug("configKey=".json_encode($configKey));

        $openssl = new HeepayOpenssl($configKey);
        $privateDeData = $openssl->decrypt($data, 2); //私钥解密

        $this->echoDebug("要解密的数据=".$data."解密的私钥=".$respri."privateDeData=".$dcyCont."解密状态=".openssl_error_string()."解密后的数据=".$privateDeData);
		return $privateDeData;
	}
	...
	function parserJSONSignData($request, $responseContent, $responseJSON) {
        ...
        $encrypted = $this->rsaDecrypt($responseJSON->data,$this->rsaPrivateKey, $this->postCharset);
		$signData->sign = $this->parserJSONSign($responseJSON);
		$signData->signSourceData = json_decode($encrypted);

        $this->echoDebug("解密后的Data数据=".$signData->signSourceData);
		return $signData;
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->heepayPublicKey) || !$this->checkEmpty($this->heepayPublicKey)) {
