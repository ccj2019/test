...
class AgreementAction extends MCommonAction {
	...
	$pagesize=10;
	  ...
	  $sql="SELECT Count(*)  FROM lzh_borrow_info A JOIN lzh_borrow_investor B  on A.id = B.borrow_id WHERE (A.borrow_status in (6,7)) and A.loan_certificate != '2' and ";
     ...
     $sql.=" (B.is_hetong = 1 OR B.add_time > 1548664779)  AND";
...
$sql="SELECT B.borrow_id,B.id as id,A.borrow_name,B.is_hetong,B.add_time  FROM lzh_borrow_info A JOIN lzh_borrow_investor B  on A.id = B.borrow_id WHERE (A.borrow_status in (6,7)) and A.loan_certificate != '2' and " ;
...
// echo $Model->getlastsql().'(1)';die;
		...
		$srt= '<div class="navslis">   <a href="/member/Agreement/?p=1&is_hetong='.$is_hetong.'">首页</a> ';
		...
		if($page<2){
			...
			$pagez=2;
				...
				if($z>0){
					...
					$srt.=' <a href="/member/Agreement/?p='.$z.'&is_hetong='.$is_hetong.'" class="nl">上一页</a>    &nbsp;';
				...
				$p++; 
					...
					for($i=1;$i<=$pagez;$i++){
						...
						if($xiaz<=$page){
					    	...
					    	$srt.='<a href="/member/Agreement?p='.$xiaz.'&is_hetong='.$is_hetong.'">'.$xiaz.'</a>  &nbsp;';
				...
				if($lt<$page){
					...
					$srt.='<a href="/member/Agreement/?p='.$lt.'&is_hetong='.$is_hetong.'" class="nl">下一页</a>    &nbsp;';

				}

		$srt.='<a href="/member/Agreement?p='.$page.'&is_hetong='.$is_hetong.'">尾页</a></div>';
...
//	var_dump($datadc['idContent'],$token,1);die;
